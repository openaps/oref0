...
ls | while read participant; do 
    ...
    cat treatments_*.js-cn --stream 'fromstream(1|truncate_stream(inputs))' | while read line; do 
    ...
    cat entries_*.json | jq -cn --stream 'fromstream(1|truncate_stream(inputs))' | while read line; do

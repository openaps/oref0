...
const generate = require('../dist/autotune-prep').default
...
if (!module.parent) {
    ...
    const pumphistory_input = inputs[0]
    const profile_input = inputs[1]
    const glucose_input = inputs[2]
    const pumpprofile_input = inputs[3]
    const carb_input = inputs[4]
    ...
    if (typeof profile_data.carb_ratio === 'undefined' || profile_data.carb_ratio < 2) {
        if (typeof pumpprofile_data.carb_ratio === 'undefined' || pumpprofile_data.carb_ratio < 2) {
            console.log(
                `{ "carbs": 0, "mealCOB": 0, "reason": "carb_ratios ${profile_data.carb_ratio} and ${pumpprofile_data.carb_ratio} out of bounds" }`
            )
    ...
    pumphistory_data.sort((a, b) => new Date(a.created_at) >= new Date(b.created_at))
    ...
    const prepped_glucose = generate(inputs)

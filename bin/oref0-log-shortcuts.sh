...
# add crontab entries
grep -q network-looplog ~/.bash_profile 2>/dev/null || echo "alias network-looplog="'"tail -n 100 -F /var/log/openaps/network.log"' >> ~/.bash_profile
...
grep -q autotune-looplog ~/.bash_profile || echo "alias autotune-looplog="'"tail -n 100 -F /var/log/openaps/autotune.log"' >> ~/.bash_profile
...
grep -q edit-pref ~/.bash_profile || echo "alias edit-pref="'"cd ~/myopenaps && vi preferences.json"' >> ~/.bash_profile
grep -q cat-wifi ~/.bash_profile || echo "alias cat-wifi="'"cat /etc/wpa_supplicant/wpa_supplicant.conf"' >> ~/.bash_profile
grep -q edit-wifi ~/.bash_profile || echo "alias edit-wifi="'"vi /etc/wpa_supplicant/wpa_supplicant.conf"' >> ~/.bash_profile

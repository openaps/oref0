...
function get_ns_bg {
    ...
    if ! find cgm/ -mmin -54 | egrep -q cgm/ns-glucose-24h.json \
        ...
        oref0-get-ns-entries cgm/ns-glucose-24h.json $NIGHTSCOUT_HOST $API_SECRET 24 2>&1 >cgm/ns-glucose-24h.json
    fi
    #nightscout ns $NIGHTSCOUT_HOST $API_SECRET oref0_glucose_since -1hour > cgm/ns-glucose-1h.json
    oref0-get-ns-entries cgm/ns-glucose-1h.json $NIGHTSCOUT_HOST $API_SECRET 1 2>&1 >cgm/ns-glucose-1h.json

...
# add crontab entries
    ...
    if [[ $ENABLE =~ autotune ]]; then
        ...
        (crontab -l; crontab -l | grep -q "oref0-autotune -d=$directory -n=$NIGHTSCOUT_HOST" || echo "5 0 * * * oref0-ns-autotune -d=$directory -n=$NIGHTSCOUT_HOST && cp $directory/autotune/profile.json $directory/settings/autotune.json | tee -a /var/log/openaps/autotune.log") | crontab -
    fi
    if [[ "$ttyport" =~ "spi" ]]; then

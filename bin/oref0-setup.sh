...
#    echo
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        if [[ $ENABLE =~ autotune ]]; then
            ...
            (crontab -l; crontab -l | grep -q "oref0-autotune -d=$directory -n=$NIGHTSCOUT_HOST" || echo "5 4 * * * ( oref0-autotune -d=$directory -n=$NIGHTSCOUT_HOST && cat $directory/autotune/profile.json | grep -q start && cat $directory/autotune/profile.json > $directory/settings/autotune.json) 2>&1 | tee -a /var/log/openaps/autotune.log") | crontab -
        fi
        if [[ "$ttyport" =~ "spi" ]]; then

...
#fi
    ...
    if [[ ! -z "$BT_PEB" || ! -z "$BT_MAC" || ${CGM,,} =~ "shareble" ]]; then
        ...
        if [ "$bluetoothdversioncompare" -eq 1 ]; then
            cd $HOME/src/ && rm -rf bluez-5.48.tar.gz && wget -4 https://www.kernel.org/pub/linux/bluetooth/bluez-5.48.tar.gz && tar xvfz bluez-5.48.tar.gz || die "Couldn't download bluez"

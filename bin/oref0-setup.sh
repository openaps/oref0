...
if [[ ${CGM,,} =~ "shareble" ]]; then
    (crontab -l; crontab -l | grep -q "cd $directory-cgm-loop && ps aux | grep -v grep | grep -q 'openaps monitor-cgm'" || echo "* * * * * cd $directory-cgm-loop && ps aux | grep -v grep | grep -q 'openaps monitor-cgm' || ( date; openaps monitor-cgm; cp -up monitor/glucose-raw-merge.json $directory/cgm/glucose.json ; cat cgm/glucose.json | json -a sgv dateString | head -1; cp -up $directory/cgm/glucose.json $directory/monitor/glucose.json ) | tee -a /var/log/openaps/cgm-loop.log") | crontab -
elif ! [[ ${CGM,,} =~ "mdt" ]]; then

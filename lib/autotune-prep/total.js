...
function diaCarbs(opts, time) {
    ...
    for (var i=bucketed_data.length-5; i > 0; --i) {
        ...
        treatments.forEach(function(treatment) {
            // consider treatments within the last 2*dia in case they're stacked
            var dia_ago = bgTime - 2*profile_data.dia*60*60*1000;
            ...
            if (treatmentTime > dia_ago && treatmentTime <= bgTime) {
                if (treatment.carbs >= 1) {
                    ...
                    if (mealCOB == 0) {
                        mealCarbs = 0;
                        //console.error("clearing mealCarbs");
                    }
                }
        ...
        if (typeof(bucketed_data[i].glucose) == 'undefined' && typeof(bucketed_data[i].sgv) != 'undefined') {
            bucketed_data[i].glucose = bucketed_data[i].sgv;
        }

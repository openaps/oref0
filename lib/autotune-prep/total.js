...
function diaCarbs(opts, time) {
    ...
    // TODO: refactor to do like lib/determine-basal/cob-autosens.js and bucketize data and for loop through it to get deltas and deviations
    glucose_data.forEach(function(glucose_datum) {
        ...
        // TODO: calculate deltas and deviations here
        // TODO: if mealCOB is zero but all deviations since hitting COB=0 are positive, assign those data points to csf_glucose_data
        ...
        if (mealCOB > 0) {
            console.error("MealCOB: ",mealCOB.toFixed(1)," at ",bgDate);
        ...
        console.error("basal BGI: ",basalBgi.toFixed(1),", BGI: ",bgi.toFixed(1)," at ",bgDate);
        // When BGI is positive (insulin activity is negative) we want to use that data to tune basals
        // When BGI is smaller than about 1/3 of basalBGI, we want to use that data to tune basals
        // When BGI is negative and more than about 1/3 of basalBGI, we can use that data to tune ISF
        if (basalBgi > -3 * bgi) {

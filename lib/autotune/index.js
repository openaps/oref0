...
function tuneAllTheThings (inputs) {
    ...
    //console.error(ISF);
    var carbRatio = previousAutotune.carbRatio;
    //console.error(carbRatio);
    var CSF = ISF / carbRatio;
    // conditional on there being a pump profile; if not then skip
    if (pumpProfile) { pump_ISFProfile = pumpProfile.ISFProfile; }
    if (pump_ISFProfile && pump_ISFProfile.sensitivities[0]) {
        pumpISF = pump_ISFProfile.sensitivities[0].sensitivity;
        pump_carbRatio = pumpProfile.carbRatio;
        pumpCSF = pumpISF / pump_carbRatio;
    }
    //console.error(CSF);
    ...
    if (pumpBasalProfile && pumpBasalProfile[0]) {
        for (var hour=0; hour < 24; hour++) {
            ...
            autotune_max = pumpProfile.autosens_max;
            autotune_min = pumpProfile.autosens_min;
            var maxRate = hourlyPumpProfile[hour].rate * autotune_max;
            var minRate = hourlyPumpProfile[hour].rate * autotune_min;
    ...
    if (typeof(pumpISF) !== 'undefined') {
        ...
        var maxISF = pumpISF * autotune_min;
        // high autosens ratio = low ISF
        var minISF = pumpISF * autotune_max;
        if (newISF > maxISF) {
            console.error("Limiting ISF to",maxISF.toFixed(2),"(which is",autotune_min"* pump ISF of",pumpISF,")");
            newISF = maxISF;
        } else if (newISF < minISF) {
            console.error("Limiting ISF to",minISF.toFixed(2),"(which is",autotune_max"* pump ISF of",pumpISF,")");
            newISF = minISF;
        }
    }
    ...
    if (typeof(pumpCSF) !== 'undefined') {
        var maxCSF = pumpCSF * autotune_max;
        var minCSF = pumpCSF * autotune_min;
        if (newCSF > maxCSF) {
            console.error("Limiting CSF to",maxCSF.toFixed(2),"(which is",autotune_min"* pump CSF of",pumpCSF,")");
            newCSF = maxCSF;
        } else if (newCSF < minCSF) {
            console.error("Limiting CSF to",minCSF.toFixed(2),"(which is",autotune_min"* pump CSF of",pumpCSF,")");
            newCSF = minCSF;
    ...
    autotuneOutput.carbRatio = carbRatio;

...
function generate (inputs) {
    ...
    for (var i=0; i < csf_glucose.length; ++i) {
        ...
        if ( csf_glucose[i].mealAbsorption === "start" ) {
            deviations = 0;
            mealCarbs = csf_glucose[i].mealCarbs;
            ...
            deviations += Math.max(previous_autotune.min_5m_carbimpact,parseFloat(csf_glucose[i].deviation));
            mealCarbs = Math.max(mealCarbs, csf_glucose[i].mealCarbs);
        }
    }

function generate (inputs) {
    ...
    var pump_carb_ratio = pumpprofile.carb_ratio;
    var pumpCSF = pumpISF / pump_carb_ratio;
    //console.error(csf);
    ...
    if (pumpbasalprofile[0]) {
        for (var hour=0; hour < 24; hour++) {
            ...
            if (hourlybasalprofile[hour].rate > maxrate ) {
                console.error("Limiting hour",hour,"basal to",maxrate.toFixed(2),"(20% above pump basal of",hourlypumpprofile[hour].rate,")");
                hourlybasalprofile[hour].rate = hourlypumpprofile[hour].rate * 1.2;
            } else if (hourlybasalprofile[hour].rate < minrate ) {
                console.error("Limiting hour",hour,"basal to",minrate.toFixed(2),"(20% below pump basal of",hourlypumpprofile[hour].rate,")");
                hourlybasalprofile[hour].rate = hourlypumpprofile[hour].rate / 1.2;
            }
    ...
    if (pumpISF) {
        ...
        if (newISF > maxISF) {
            console.error("Limiting ISF to",maxISF.toFixed(2),"(20% above pump ISF of",pumpISF,")");
            newISF = maxISF;
        } else if (newISF < minISF) {
            console.error("Limiting ISF to",minISF.toFixed(2),"(20% below pump ISF of",pumpISF,")");
            newISF = minISF;
        }
    }
    ...
    if (pumpCSF) {
        ...
        if (newCSF > maxCSF) {
            console.error("Limiting CSF to",maxCSF.toFixed(2),"(20% above pump CSF of",pumpCSF,")");
            newCSF = maxCSF;
        } else if (newCSF < minCSF) {
            console.error("Limiting CSF to",minCSF.toFixed(2),"(20% below pump CSF of",pumpCSF,")");
            newCSF = minCSF;

...
function tuneAllTheThings (inputs) {
    ...
    if (! previousAutotune.useCustomPeakTime && previousAutotune.curve === "ultra-rapid" ) {
        peak = 55;
    }
    ...
    if ( best < 2 ) {
        if ( diaDeviations[1].devSquared < currentDIADevSq ) {
            newDIA = diaDeviations[1].dia;
        }
    } else if ( best > 2 ) {
        if ( diaDeviations[3].devSquared < currentDIADevSq ) {
            newDIA = diaDeviations[3].dia;
        }
    }
    if ( newDIA != DIA ) {
        console.error("Adjusting DIA from",DIA,"to",newDIA,"hours");
    }
    ...
    if ( best < 2 ) {
        if ( peakDeviations[1].devSquared < currentPeakDevSq ) {
            newPeak = peakDeviations[1].peak;
        }
    } else if ( best > 2 ) {
        if ( peakDeviations[3].devSquared < currentPeakDevSq ) {

...
//console.error("skipping:",glucose_data[i].glucose,glucose_data[i-1].glucose);
    ...
    for (i=3; i < bucketed_data.length; ++i) {
        ...
        // add an extra negative deviation if BG is below 60 mg/dL
        if ( profile.hypoThreshold && bg < profile.hypoThreshold ) {
            // add a -10 deviation for every 5m spent below hypoThreshold
            var hypoDeviation=-10
            process.stderr.write("*");
            //console.error(tempDeviation)
            deviations.push(hypoDeviation);
        }

...
function detectSensitivityandCarbAbsorption(inputs) {
    ...
    for (var i=1; i < glucose_data.length; ++i) {
        ...
        // only consider last 1.5 hours of data in CI mode
        // this allows us to calculate deviations for just over an hour
        if (typeof ciTime) {
            hoursAgo = (ciTime-bgTime)/(60*60*1000);
            if (hoursAgo > 1.5 || hoursAgo < 0) {
                continue;
            }
        }
    ...
    for (var i=0; i < bucketed_data.length-3; ++i) {
        ...
        if (i==0) {
            ...
            if (ciTime > bgTime) {
                //console.error(currentDeviation);
            }
            ...
            if (avgDeviation > maxDeviation) {
                minDeviationSlope = Math.min(minDeviationSlope, deviationSlope);

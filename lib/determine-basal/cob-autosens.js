...
function detectSensitivityandCarbAbsorption(inputs) {
    ...
    for (var i=0; i < bucketed_data.length-3; ++i) {
        ...
        if (i==0) {
            ...
            if (currentDeviation > profile.min_5m_carbimpact) {
                console.error("currentDeviation",currentDeviation,"> min_5m_carbimpact",profile.min_5m_carbimpact);
            }
        }
        ...
        if(bgTime > mealTime) {
            ...
            ci = Math.max(deviation, currentDeviation, profile.min_5m_carbimpact);

...
function detectSensitivityandCarbAbsorption(inputs) {
    ...
    for (var i=0; i < glucose_data.length-3; ++i) {
        ...
        if(bgTime > mealTime) {
            //console.error(bgTime, deviation)
            // and deviation is positive
            if (deviation > 0) {
                // figure out how many carbs that represents
                //console.error(delta)
                absorbed = deviation * profile.carb_ratio / profile.sens;
                //console.error(absorbed);
                // and add that to the running total carbsAbsorbed
                carbsAbsorbed += absorbed;
                //console.error(carbsAbsorbed);
            }
        }
    }
    //console.error("");
    ...
    for (var i=0.9; i > 0.1; i = i - 0.02) {
        if ( percentile(deviations, (i+0.02)) >= 0 && percentile(deviations, i) < 0 ) {
            ...
            console.error(100*i.toFixed(2)+"% of non-meal deviations negative (target 45%-50%)");
        }
    }
    ...
    if(pSensitive < 0) { // sensitive
        ...
        process.stderr.write("Sensitivity normal: ");
    }

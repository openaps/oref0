...
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    calcreason += "BG-accel: " + round(bg_acceleration,3) + ", PF-minutes: " + parabola_fit_minutes + ", PF-corr: " + convert_bg(parabola_fit_correlation,profile) + ", PF-nextDelta: " + convert_bg(parabola_fit_next_delta,profile) + ", PF-lastDelta: " + convert_bg(parabola_fit_last_delta,profile) +  ", regular Delta: " + convert_bg(glucose_status.delta,profile);
    console.error(ppdebug + calcreason + " , Weights Accel/Brake: " + profile.bgAccel_ISF_weight + " / " + profile.bgBrake_ISF_weight)
    calcreason = ", Parabolic Fit, last\u0394: " +convert_bg(parabola_fit_last_delta,profile) + ", next\u0394: " + convert_bg(parabola_fit_next_delta,profile) + ", reg\u0394: " + convert_bg(glucose_status.delta,profile);
    ...
    if (meal_data.mealCOB>0 && !profile.enableautoisf_with_COB) {
        ...
        if ( fit_corr<0.9 ) {
            console.error("accel_ISF adaptation by-passed as correlation " + round(fit_corr,3) + " is too low");
        } else {
            ...
            console.error("Calc result for acce_ISF:" + round(acce_ISF,2));
            if ( acce_ISF != 1 ) {
            sens_modified = true;
            }
    ...
    if ( sens_modified ) {
        ...
        isfreason += ", autoISF, Ratio: " + round(liftISF,2) + ", ISF: " + convert_bg(sens,profile) + calcreason;
    }
...
/*                 isfreason += "Floating Carbs:, CID " + round(cid,1) + ", MealCarbs " + round(meal_data.carbs,1) + ", Not     isfreason += Floating:, CID " + round(nfcid,1) + ", MealCOB " + round(meal_data.mealCOB, 1) + ", "; */

...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data) {
    ...
    } else if (profile.temptargetSet && target_bg > 100) {
        console.error("SMB disabled due to high temptarget");
        enableSMB=false;
    // enable SMB/UAM (if enabled in preferences) while we have COB
        ...
        if (meal_data.bwCarbs) {
            ...
            console.error("SMB enabled for COB");
            enableSMB=true;
        }
    ...
    // (6 hours is defined in carbWindow in lib/meal/total.js)
        ...
        if (meal_data.bwCarbs) {
            ...
            console.error("SMB enabled for carbs");
            enableSMB=true;
        }
    // enable SMB/UAM (if enabled in preferences) if a low temptarget is set
        ...
        if (meal_data.bwFound) {
            if (A52_risk_enable) {
                console.error("Warning: SMB enabled within 24h of using Bolus Wizard: be sure to easy bolus 30s before using Bolus Wizard")
                enableSMB=true;
            } else {
                console.error("enableSMB_with_temptarget not supported within 24h of using Bolus Wizard");
            }
        } else {
            enableSMB=true;
        }
    // enable SMB/UAM if always-on (unless previously disabled for high temptarget)
        ...
        if (meal_data.bwFound) {
            if (A52_risk_enable) {
                ...
                console.error("enableSMB_always not supported within 24h of using Bolus Wizard");
            }
        } else {
            enableSMB=true;
        }

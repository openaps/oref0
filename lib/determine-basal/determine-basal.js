...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data) {
    ...
    if ( bg > target_bg && profile.adjust_targets_when_high ) {
        // with target=100, as BG rises from 100 to 130, adjustedTarget drops from 100 to 85
        adjustedTargetBG = Math.max(85,(bg - target_bg)/2);
        // if eventualBG, naiveEventualBG, and target_bg aren't all above adjustedTarget, don’t use it
        if (eventualBG > adjustedTargetBG && naiveEventualBG > adjustedTargetBG && target_bg > adjustedTargetBG) {
            console.error("Adjusting target_bg from",target_bg,"to",adjustedTargetBG);
            target_bg = adjustedTarget;
        }
    }
    ...
    try {
        iobArray.forEach(function(iobTick) {
            ...
            if ( IOBpredBGs.length > 9 && (IOBpredBG < minIOBPredBG) ) { minIOBPredBG = IOBpredBG; }

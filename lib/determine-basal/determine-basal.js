      ...
      console.error("SMB fraction of InsReq: "+profile.smb_delivery_ratio);
        ...
        if (meal_data.mealCOB==0 && dura05>=10) {
            if (avg05 > target_bg) {
                ...
                console.error("autoISF reports "+sens+" did not do it for "+dura05+"min; go more aggressive by "+round(levelISF,2));
	            ...
	            console.error("autoISF reports sens is now "+sens);
                isfreason += "autoISF -> " +sens + ", Ratio " +liftISF + "; "
            } else {
...
// Adjust ISF based on sensitivityRatio
    ...
    if (typeof autosens_data !== 'undefined' && autosens_data) {
        ...
        //process.stderr.write(" (autosens ratio "+sensitivityRatio+")");
        isfreason += "Autosense: Ratio " + sensitivityRatio + ", ISF "+profile_sens+" -> " +sens  + "; "
...
// Begin core dosing logic: check for situations requiring low or high temps, and return appropriate temp after first match
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            if (profile.auto_isf===false) {
              console.error("autoISF disabled, don't adjust SMB range");
              var smb_max_range = 1;
            } else {

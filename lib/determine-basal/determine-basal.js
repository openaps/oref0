...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, offline, meal_data, setTempBasal) {
    ...
    //console.error("minDelta", minDelta, "bgi", bgi, "meal_data.carbs/profile.carb_ratio", meal_data.carbs/profile.carb_ratio, "meal_data.boluses", meal_data.boluses, "iob_data.iob", iob_data.iob);
    ...
    if (minDelta > Math.max(0, bgi) && (meal_data.carbs/profile.carb_ratio > meal_data.boluses + basaliob)) {
        ...
        eventualBG = Math.max(bg,eventualBG);
        rT.eventualBG = eventualBG;
        // set min_bg to 80, eating-soon style 
        profile.min_bg = 80;
        target_bg = (profile.min_bg + profile.max_bg) / 2;
        expectedDelta = Math.round(( bgi + ( target_bg - eventualBG ) / ( profile.dia * 60 / 5 ) )*10)/10;
        rT.mealAssist = "Carbs: " + meal_data.carbs + " Boluses: " + meal_data.boluses + " Target: " + target_bg;
    ...
    if (eventualBG < profile.min_bg) { // if eventual BG is below target:
        ...
        if (eventualBG < profile.min_bg) {
            ...
            if (snoozeBG > profile.min_bg) { // if adding back in the bolus contribution BG would be above min
                ...
                // use greater of snoozeBG or eventualBG to more gradually ramp in any counteraction of the user's boluses
                var insulinReq = Math.min(0, (Math.max(snoozeBG,eventualBG) - target_bg) / profile.sens);
                ...
                if (typeof currenttemp.rate !== 'undefined' && (currenttemp.duration > 0 && rate < currenttemp.rate + 0.2 && rate > currenttemp.rate - 0.1)) {

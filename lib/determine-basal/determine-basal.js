...
*/
    ...
    // TODO: generate predicted future BGs based on IOB, COB, and current absortpion rate (deviation)
    //console.error(meal_data.mealCOB);
    //if (meal_data.mealCOB > iob_data.iob*profile.carb_ratio) {}
    ...
    if (meal_data.mealCOB > 0 && minDelta - bgi > 0) {
        ...
        if (typeof iobArray !== 'undefined' ) {
            iobArray.forEach(function(iobTick) {
                ...
                predBGs.push(predBG);
                if ( (predBGI+predCI < 0) && (predBG < minPredBG) ) { minPredBG = predBG; }
            ...
            rT.eventualBG = eventualBG;
            if (eventualBG < minPredBG) { minPredBG = eventualBG; }
            //if (minPredBG < bg) {
                snoozeBG = Math.round(minPredBG);
    ...
    rT.reason="COB: " + meal_data.mealCOB + ", Target: " + target_bg + ", Deviation: " + deviation + ", BGI: " + bgi + "; ";
    if (bg < threshold) { // low glucose suspend mode: BG is < ~80
        rT.reason += "BG " + bg + "<" + threshold;
        if ((glucose_status.delta <= 0 && minDelta <= 0) || (glucose_status.delta < expectedDelta && minDelta < expectedDelta)) {

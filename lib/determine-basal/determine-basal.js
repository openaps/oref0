    ...
    if (meal_data.mealCOB > 0 && !profile.enableautoisf_with_COB) {
        ...
        if  (!profile.enable_BG_acceleration) {
            ...
            if ( fit_corr <= 0.9 ) {
                ...
                calcreason += ", Parabolic Fit, " + fitreason + ", reg\u0394: " + convert_bg(glucose_status.delta,profile) + ", last\u0394: " +convert_bg(parabola_fit_last_delta,profile) + ", next\u0394: " + convert_bg(parabola_fit_next_delta,profile) + ", Corr " + round(parabola_fit_correlation,3) + ", BG-Accel: " + round(bg_acce,2);
        ...
        if ( bg_ISF < 1 && acce_ISF > 1 ) {                                                                             // mod V14j
            ...
            bgisfreason = "(lifted by " + acce_ISF + ")";
        ...
        if (bg_ISF < 1) {                                                                                                      // mod V14j
            ...
            if ( liftISF < profile.autoisf_min ) {                                                                               // mod V14j
                ...
                console.error(isfadaptionreason);
                bgisfreason = "(limited to " + profile.autoisf_min + ")";
            ...
            earlyreason = ", bg-ISF Ratio" + bgisfreason + ": " + round(bg_ISF,2) + ", ISF: " + convert_bg(earlysens,profile) ;
            isfreason +=  earlyreason ;
            bgisfreason = earlyreason ;
            return earlysens;
        } else if ( bg_ISF > 1 ) {
            sens_modified = true;
            isfreason +=  ", bg-ISF Ratio" + bgisfreason + ": " + round(bg_ISF,2);
        }
        ...
        if (meal_data.mealCOB>0 && !profile.enableautoisf_with_COB) {
            ...
            sens_modified = true;
            duraisfreason = ", dura-ISF Ratio: " + round(dura_ISF,2);
        ...
        isfreason += ", Duration: " + dura05 + ", Avg: " + convert_bg(avg05,profile) + bgisfreason +  ppisfreason + deltaisfreason + duraisfreason + ", Ratio: " + round(liftISF,2) + ", ISF: " + convert_bg(sens,profile);

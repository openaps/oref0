...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, setTempBasal) {
    ...
    if (typeof iobArray !== 'undefined' ) {
        iobArray.forEach(function(iobTick) {
            ...
            COBpredBG = COBpredBGs[COBpredBGs.length-1] + predBGI + predCI;
            aCOBpredBG = aCOBpredBGs[aCOBpredBGs.length-1] + predBGI + predACI;
            //console.error(predBGI, predCI, predBG);
            IOBpredBGs.push(Math.round(IOBpredBG));
            COBpredBGs.push(Math.round(COBpredBG));
            aCOBpredBGs.push(Math.round(aCOBpredBG));
    ...
    IOBpredBGs.forEach(function(p, i, theArray) {
        theArray[i] = Math.min(401,Math.max(39,p));
    });

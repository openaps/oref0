    ...
    */
    ...
    if (basaliob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && profile.current_basal >= 0.3 && bg > threshold) {
            ...
            // if IOB covers less than 2/3 of COB, microBolus 1/2 the insulinReq
            // (or enough insulin to cover 2/3 of COB, or maxBolus, whichever is smallest)
            if ( iob_data.iob < 2/3 * mealInsulinReq ) {
                initialMealInsulinReq = round(2/3*mealInsulinReq-iob_data.iob,1);

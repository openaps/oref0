...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, setTempBasal) {
    ...
    var high = profile.max_bg + ( basal * (profile.dia/2) * sens );
    ...
    if (bg < threshold) { // low glucose suspend mode: BG is < ~80
        ...
        if ((glucose_status.delta <= 0 && glucose_status.avgdelta <= 0) || (glucose_status.delta < expectedDelta && glucose_status.avgdelta < expectedDelta)) {
            ...
            return setTempBasal(0, 30, profile, rT);
        }
        ...
        return setTempBasal(basal, 30, profile, rT);
        /*
        if (currenttemp.rate > basal) { // if a high-temp is running
            rT.reason += ", cancel high temp";
            return setTempBasal(0, 0, profile, rT); // cancel high temp
            ...
            return setTempBasal(0, 0, profile, rT); // cancel low temp
        }
    ...
    if (remainingMealBolus > 0 && snoozeBG + remainingMealBolus*sens > min_bg && minDelta > expectedDelta) {
        ...
        //return setTempBasal(rate, 30, profile, rT);
        ...
        //if (mealAssist === true) {
            ...
            if (minDelta > expectedDelta && minDelta > 0) {
                ...
                if (currenttemp.duration > 0) { // if there is currently any temp basal running
                    rT.reason +== "; cancel";
                    return setTempBasal(0, 0, profile, rT); // cancel temp
                } else {
                */
                    rT.reason += "; setting current basal of " + basal + " as temp";
                    return setTempBasal(basal, 30, profile, rT);
    ...
    if (basaliob > max_iob) {
        ...
        //return setTempBasal(0, 0, profile, rT);
        rT.reason += "; setting current basal of " + basal + " as temp";
        return setTempBasal(basal, 30, profile, rT);

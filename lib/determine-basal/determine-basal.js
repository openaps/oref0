...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data) {
    ...
    try {
        iobArray.forEach(function(iobTick) {
            ...
            remainingCItotal += remainingCI;
            predCItotal += predCI;
            predACItotal += predACI;
            process.stderr.write(round(predCI+remainingCI,1)+" ");
    ...
    console.error("");
    console.error(round(remainingCItotal,2),round(remainingCItotal/csf,2),round(remainingCarbs,2),round(totalCA,2),round(remainingCItotal/csf+totalCA,2));
    console.error(round(predCItotal+remainingCItotal,2),round((predCItotal+remainingCItotal)/csf,2));
    console.error(round(predCItotal,2),round((predCItotal)/csf,2),round(totalCI,2),round(totalCI/csf,2));
    console.error(round(predACItotal,2),round(predACItotal/csf,2));
    rT.predBGs = {};

...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data) {
    ...
    if (meal_data.carbs) {
        ...
        var minMinUAMavg = Math.min( minUAMPredBG, avgPredBG );
        // if UAM is disabled, use max of minIOBPredBG, minCOBPredBG
        if ( ! enableUAM && minCOBPredBG < 400 ) {
            minPredBG = round(Math.max(minIOBPredBG, minCOBPredBG));
        // otherwise, if we still have COB, do a weighed averaging based on how many carbs remain as COB
        } else if ( minCOBPredBG < 400 ) {
            fractionCarbsLeft = meal_data.mealCOB/meal_data.carbs;
            blendedMinPredBG = fractionCarbsLeft*minCOBPredBG + (1-fractionCarbsLeft)*minMinUAMavg;
            minPredBG = round(Math.max(minIOBPredBG, blendedMinPredBG));
        // if carbs have been entered, but have expired, use lower of minUAMPredBG or avgPredBG
        } else {
            minPredBG = minMinUAMavg;
        }
    // in pure UAM mode, use the higher of the minIOBPredBG,minUAMPredBG
    } else if ( enableUAM ) {
    ...
    rT.IOB=iob_data.iob;
    rT.reason="COB: " + meal_data.mealCOB + ", Dev: " + deviation + ", BGI: " + bgi + ", ISF: " + convert_bg(sens, profile) + ", Target: " + convert_bg(target_bg, profile) + ", IOBpredBG " + convert_bg(lastIOBpredBG, profile) + ", minPredBG " + convert_bg(minPredBG, profile);
    if (lastCOBpredBG > 0) {

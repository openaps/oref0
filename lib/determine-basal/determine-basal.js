...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, offline, meal_data, setTempBasal) {
    ...
    var netbasalinsulin = iob_data.netbasalinsulin;
    ...
    var high = profile.max_bg + ( profile.current_basal * (profile.dia/2) * profile.sens );
    if ( bg > high && minDelta > Math.max(0,bgi) ) {
        wtfAssist=1;
    }
    // minDelta is > 7 and devation is > 25 wtf-assist and meal-assist
    if ( deviation > 25 && minDelta > 7 ) {
        wtfAssist=1;
        mealAssist=1;
    } else {
        // phase in mealAssist, as a fraction
        mealAssist = Math.max(0, Math.round( Math.min(deviation/25,minDelta/7)*100)/100 );
    }
    var remainingMealBolus = 1.1 * meal_data.carbs/profile.carb_ratio - ( meal_data.boluses + Math.max(0,netbasalinsulin) )
    if (
    ...
    if (wtfAssist > 0) {
        ...
        mealAssistPct = mealAssist*100;
        wtfAssistPct = wtfAssist*100;
    ...
    if (eventualBG < min_bg) { // if eventual BG is below target:
        ...
        if (eventualBG < min_bg) {
            ...
            if (remainingMealBolus > 0 && snoozeBG + remainingMealBolus*profile.sens > min_bg) {
                // simulate an extended bolus to deliver the remainder over DIA (so 30m is 0.5x remainder/dia)
                // TODO: reduce insulinReq proportionally as avgDelta drops from 0 down to expectedDelta
                var insulinReq = 0.5 * remainingMealBolus / profile.dia;
            ...
            // otherwise, if we've bolused recently, we can snooze until the bolus IOB decays (at double speed)

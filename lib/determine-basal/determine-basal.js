...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data) {
    ...
    // make sure minIOBPredBG is at least 40, in order to ignore any other minPredBGs lower than that
    minIOBPredBG = Math.max(40, minIOBPredBG);
    // if only minUAMPredBG or minCOBPredBG has been set, use the higher of that and minIOBPredBG)
    minPredBG = round(minIOBPredBG);
    if ( minUAMPredBG < 400 ) {
        minPredBG = round(Math.max(minPredBG, minUAMPredBG));
    }
    if ( minCOBPredBG < 400 ) {
        minPredBG = round(Math.max(minPredBG, minCOBPredBG));
    }
    // if both minUAMPredBG or minCOBPredBG are set, and are higher than minIOBPredBG,
    // do a weighed averaging based on how many carbs remain as COB
    if ( minUAMPredBG < 400 && minCOBPredBG < 400 && minUAMPredBG > minIOBPredBG && minCOBPredBG > minIOBPredBG) {
        ...
        minPredBG = round(Math.max(minIOBPredBG, blendedMinPredBG));
    }
    console.error("minPredBG:",minPredBG,"minIOBPredBG:",minIOBPredBG,"minCOBPredBG:",minCOBPredBG,"minUAMPredBG:",minUAMPredBG,"COB:",meal_data.mealCOB,"carbs:",meal_data.carbs);

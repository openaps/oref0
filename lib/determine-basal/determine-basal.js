...
var calcreason = ", Parabolic Fit"; // parabolic fit debugging
...
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    if (meal_data.mealCOB>0 && !profile.enableautoisf_with_COB) {
        ...
        if ( glucose_status.parabola_fit_correlation < 0.9 ) {
            console.error("accel_ISF adaptation by-passed as correlation " + round(fit_corr,3) + " is too low");
        } else {
            calcreason += ", reg\u0394: " + convert_bg(glucose_status.delta,profile) + ", last\u0394: " +convert_bg(parabola_fit_last_delta,profile) + ", next\u0394: " + convert_bg(parabola_fit_next_delta,profile) + ", Corr " + round(parabola_fit_correlation,3) + ", BG-Accel: " + round(bg_acceleration,2);

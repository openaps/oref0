...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, offline, meal_data, setTempBasal) {
    ...
    if (
        ...
        (meal_data.carbs > 0 && (1.1 * meal_data.carbs/profile.carb_ratio > meal_data.boluses + netbasalinsulin))
      ) {
        mealAssist=true;
    }
    ...
    if (bg < threshold) { // low glucose suspend mode: BG is < ~80
        ...
        if ((glucose_status.delta <= 0 && glucose_status.avgdelta <= 0) || (glucose_status.delta < expectedDelta && glucose_status.avgdelta < expectedDelta) || bg < 60 ) {

...
// Begin core dosing logic: check for situations requiring low or high temps, and return appropriate temp after first match
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (currenttemp.duration > 15 && (round_basal(basal, profile) === round_basal(currenttemp.rate, profile))) {
            rT.reason += ", temp " + currenttemp.rate + " ~ req " + basal + "U/hr. " + isfreason;
            return rT;
        } else {
            rT.reason += "; setting current basal of " + basal + " as temp. " + isfreason;
            return tempBasalFunctions.setTempBasal(basal, 30, profile, rT, currenttemp);
        }
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            if ( !profile.use_autoisf ) {
                ...
                isfreason += "SMB Delivery Ratio " + round(smb_ratio,2) + ", ";
              }
            if ( smb_ratio > 0.5) {
        ...
        rT.reason += "temp " + currenttemp.rate + "<" + rate + "U/hr. " + isfreason;
        return tempBasalFunctions.setTempBasal(rate, 30, profile, rT, currenttemp);
    }

...
{   // #### mod 7e: added switch fr autoISF ON/OFF
    if (profile.auto_isf===false) {
        console.error("autoISF disabled in Preferences");
        return sens;
    } else if
      (typeof profile.autoisf_max === 'undefined') {
        console.error("autoISF_max not defined");
        return sens;
    } else if(profile.autoisf_max) {
      console.error("autoISF enabled in Preferences");
      console.error("autoISF hourly change: "+profile.autoisf_hourlychange);
      console.error("autoISF max: "+profile.autoisf_max);
      console.error("SMB Range Extension: "+profile.smb_max_range_extension);
      console.error("SMB fraction of InsReq: "+profile.smb_delivery_ratio);
        //var sensitivityRatio = autosens_data.ratio;
        //console.error("Sensitivity Ratio ="+sensitivityRatio+"; ");
        // #### mod 7e: added switch fr autoISF ON/OFF
        ...
        if (meal_data.mealCOB==0 && dura05>=10) {
            if (avg05 > target_bg) {
                ...
                var liftISF = round(Math.max(ISFreductionLevel, sensitivityRatio),2);  // corrected logic on 30.Jan.2021
                console.error("dura05_weight = "+dura05_weight+";  avg05_weight = "+avg05_weight+"; levelISF = "+levelISF+"; liftISF = "+liftISF);
                console.error("autoISF reports "+ convert_bg(sens,profile) +" did not do it for "+dura05+"min; go more aggressive by "+round(levelISF,2));
	            ...
	            console.error("autoISF reports sens is now " + convert_bg(sens,profile));
                isfreason += "autoISF -> " + convert_bg(sens,profile) + ", Ratio " +liftISF + "; "
            } else {
        ...
        console.error("autoISF by-passed; BG is only "+dura05+"min at level "+avg05);
        }
        console.error("autoISF has run. Sens is now " + convert_bg(sens,profile));
        return sens;
    }
}
//end autoisf
...
// Adjust ISF based on sensitivityRatio
    ...
    if (typeof autosens_data !== 'undefined' && autosens_data) {
        ...
        //process.stderr.write(" (autosens ratio "+sensitivityRatio+")");
        isfreason += "Autosense: Ratio " + sensitivityRatio + ", ISF "+ convert_bg(profile_sens,profile) +" -> " + convert_bg(sens,profile)  + "; "
    ...
    if (ci === 0) {
        ...
        if (profile.floating_carbs === true) {
            ...
            if (meal_data.carbs > 0){
                isfreason += "Floating Carbs CID:" + round(cid,1) + " / MealCarbs:" + round(meal_data.carbs,1) + " vs. Not Floating:" + round(nfcid,1) + " / MealCOB:" + round(meal_data.mealCOB, 1) + "; ";
...
// Print summary variables based on predBGs etc.
    ...
    rT.reason= isfreason + "minPredBG " + convert_bg(minPredBG, profile) + ", minGuardBG " + convert_bg(minGuardBG, profile) + ", IOBpredBG " + convert_bg(lastIOBpredBG, profile);
    if (lastCOBpredBG > 0) {
...
// Begin core dosing logic: check for situations requiring low or high temps, and return appropriate temp after first match
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            if (profile.auto_isf===false) {
              console.error("autoISF disabled, SMB range is 1");
              var smb_max_range = 1;
            } else {
            ...
            if (profile.auto_isf===false) {
                console.error("autoISF disabled, don't adjust SMB ratio");
                smb_ratio = 0.5;
            } else {

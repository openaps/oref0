...
// Adjust ISF based on sensitivityRatio
    ...
    if (typeof autosens_data !== 'undefined' && autosens_data) {
        ...
        if (profile.ISF_multiplier) {
            sens = sens * profile.ISF_multiplier;
            process.stderr.write("ISF_multiplier "+profile.ISF_multiplier+"; ");
        }
        sens = round(sens, 1);
...
// Initialize and calculate variables used for predicting BGs
    ...
    if (ci === 0) {
        ...
        if (profile.floating_carbs === true && meal_data.mealCOB > 0) {
    ...
    try {
        iobArray.forEach(function(iobTick) {
            ...
            if (profile.ZT_based_predBGs === true ) {
                predBGI = predZTBGI;
            }
    ...
    for (var i=IOBpredBGs.length-1; i > 12; i--) {
        // stop displaying normal IOBpredBGs once they flatten out; ZT_based IOBpredBGs once they're rising and above target
        if (profile.ZT_based_predBGs !== true && IOBpredBGs[i-1] !== IOBpredBGs[i]) { break; }
        else if ( (IOBpredBGs[i] < 400 && IOBpredBGs[i-1] >= IOBpredBGs[i] ) || IOBpredBGs[i] <= target_bg) { break; }
        else {
            IOBpredBGs.pop();
            if (IOBpredBG > IOBpredBGs[i-1]) { IOBpredBG = IOBpredBGs[i-1]; }
        }
    }
    ...
    if (meal_data.mealCOB > 0 && ( ci > 0 || remainingCIpeak > 0 )) {
        ...
        for (i=COBpredBGs.length-1; i > 12; i--) {
            ...
            // else { COBpredBGs.pop(); }
            if (profile.ZT_based_predBGs !== true && COBpredBGs[i-1] !== COBpredBGs[i]) { break; }
            else if ( (COBpredBGs[i] < 400 && COBpredBGs[i-1] >= COBpredBGs[i] ) || COBpredBGs[i] <= target_bg) { break; }
            else {
                COBpredBGs.pop();
                if (COBpredBG > COBpredBGs[i-1]) { COBpredBG = COBpredBGs[i-1]; }
            }
        }
    ...
    if (ci > 0 || remainingCIpeak > 0) {
        if (enableUAM) {
            ...
            for (i=UAMpredBGs.length-1; i > 12; i--) {
                if (profile.ZT_based_predBGs !== true && UAMpredBGs[i-1] !== UAMpredBGs[i]) { break; }
                else if ( (UAMpredBGs[i] < 400 && UAMpredBGs[i-1] >= UAMpredBGs[i] ) || UAMpredBGs[i] <= target_bg) { break; }
                else if (UAMpredBGs[i-1] >= UAMpredBGs[i] || UAMpredBGs[i] <= target_bg) { break; }
                else {
                    UAMpredBGs.pop();
                    if (UAMpredBG > UAMpredBGs[i-1]) { UAMpredBG = UAMpredBGs[i-1]; }
                }
            }

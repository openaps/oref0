...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile) {
    ...
    var tddReason = ", TDD: " + round(tdd, 2) + " U. ";
    ...
    if (chrisFormula == true && tdd > 0) {
        ...
        logOutPut += startLog + bgLog + afLog + formula + log + log_isfCR;
    } else if (chrisFormula == false && useDynamicCR == true) {
        logOutPut += startLog + bgLog + afLog + formula + "Dynamic ISF is off." + logCR + log_isfCR;
    } else {
        logOutPut += startLog + "Dynamic ISF is off. Dynamic CR is off." + log_isfCR ;
    }

    console.log(logOutPut);
   
    tddReason += log_isfCR + afLog + formula;

...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data) {
    ...
    if ( bg > max_bg && profile.adjust_targets_when_high ) {
        ...
        var adjustedMinBG = Math.max(85, min_bg - (bg - min_bg)/2 );
        var adjustedTargetBG = Math.max(85, target_bg - (bg - target_bg)/2 );
        var adjustedMaxBG = Math.max(85, max_bg - (bg - max_bg)/2 );
        // if eventualBG, naive_eventualBG, and target_bg aren't all above adjustedMinBG, don’t use it
        if (eventualBG > adjustedMinBG && naive_eventualBG > adjustedMinBG && min_bg > adjustedMinBG) {
            console.error("Adjusting min_bg from",min_bg,"to",adjustedMinBG);
        ...
        if (eventualBG > adjustedMaxBG && naive_eventualBG > adjustedMaxBG && max_bg > adjustedMaxBG) {
            console.error("Adjusting max_bg from",max_bg,"to",adjustedMaxBG);

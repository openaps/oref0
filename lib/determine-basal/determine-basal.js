...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data) {
    ...
    if (glucose_status.noise >= 2) {
        ...
        var rawTargetMultiplier = Math.max( 1.1, profile.rawTargetMultiplier );
        // don't allow maxRaw above 250
        var maxRaw = Math.min( 250, profile.maxRaw );
        var adjustedMinBG = round(Math.min(200, min_bg * rawTargetMultiplier ));
        var adjustedTargetBG = round(Math.min(200, target_bg * rawTargetMultiplier ));
        var adjustedMaxBG = round(Math.min(200, max_bg * rawTargetMultiplier ));

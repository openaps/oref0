...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed) {
    ...
    if (basaliob > max_iob) {
        ...
        var rate = basal + (2 * insulinReq);
        rate = round_basal(rate, profile);
        rT.insulinReq = round(insulinReq,3);
        //console.error(iob_data.lastBolusTime);
        // minutes since last bolus
        // TODO: figure out how to verify pump time matches rig time
        ...
        if (microBolusAllowed && ((profile.temptargetSet && target_bg < 100) || rT.COB)) { // && insulinReq > profile.current_basal/3) {
            // TODO: ensure that total boluses delivered over the last hour do not exceed the max basal rate
            // never bolus more than 30m worth of basal
            maxBolus = profile.current_basal/2;
            // bolus half the insulinReq, up to maxBolus
            microBolus = round(Math.min(insulinReq/2,maxBolus),1);

            rT.rate = 0;
            // calculate a long enough zero temp to eventually correct back up to target
            var smbTarget = target_bg;
            // if a low temp target is running, zero temp up to 100
            if (profile.temptargetSet) { smbTarget = Math.max(target_bg,100); }
            var worstCaseInsulinReq = (smbTarget - naive_eventualBG) / sens + insulinReq;
            var durationReq = round(60*worstCaseInsulinReq / profile.current_basal);
            console.error(naive_eventualBG, insulinReq, worstCaseInsulinReq, durationReq);
            // if no zero temp is required, cancel
            if (durationReq < 0) {
                durationReq = 0;
            // if any zero temp is required, set 30m minimum; otherwise round to 30m increments
            ...
            rT.reason += "insulinReq " + insulinReq + "; setting " + durationReq + "m zero temp"

            // TODO: allow a new bolus after every new BG reading
            // only allow one bolus every 4m for now
            var nextBolusMins = round(4-lastBolusAge,1);
            console.error("last bolus",lastBolusAge,"m ago (",iob_data.lastBolusTime,").");
            if (lastBolusAge > 4) {
                if (microBolus > 0) {
                    rT.units = microBolus;
                    rT.reason += " and microbolusing " + microBolus + "U";
                }
            } else {

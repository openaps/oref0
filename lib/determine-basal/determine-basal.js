...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime) {
    ...
    try {
        iobArray.forEach(function(iobTick) {
            ...
            // calculate predBGs with long zero temp without positive deviations
            if ( predDev < 0 ) {
                ZTpredBG = ZTpredBGs[ZTpredBGs.length-1] + predZTBGI + predDev;
            } else {
    ...
    } else if ( bg < threshold || ( minZTGuardBG < threshold && minGuardBG < threshold ) || ( minZTGuardBG < threshold && iob_data.iob > mealInsulinReq ) ) {
        var bgUndershoot;
        // predictive low glucose suspend mode: BG is / is projected to be < threshold
        if ( bg < threshold || ( minZTGuardBG < threshold && minGuardBG < threshold ) ) {
            rT.reason += "minGuardBG " + convert_bg(minGuardBG, profile) + "<" + convert_bg(threshold, profile);
            bgUndershoot = target_bg - minGuardBG;
        // zero-temp (and don't SMB) if minZTGuardBG < threshold and insufficient COB to cover IOB
        } else {
            rT.reason += "minZTGuardBG " + convert_bg(minZTGuardBG, profile) + " < " + convert_bg(threshold, profile) + ", IOB " + iob_data.iob + " > mealInsulinReq " + mealInsulinReq;
            bgUndershoot = target_bg - minZTGuardBG;
        }
    ...
    if (iob_data.iob > max_iob) {
        ...
        insulinReq = round(insulinReq,2);
        // if insulinReq would put us over max_iob, then reduce accordingly
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            // if IOB more than covers COB and microBolus would push minZTGuardBG below threshold, reduce it
            if ( iob_data.iob > mealInsulinReq ) {
                microBolus = round(Math.min( microBolus, (minZTGuardBG-threshold)/sens ),1);
            }
            // calculate a long enough zero temp to eventually correct back up to target
            ...
            //allow SMBs every SMBInterval minutes
            var SMBInterval = 3;

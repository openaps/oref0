...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data) {
    ...
    IOBpredBGs.push(bg);
    UAMpredBGs.push(bg);
    // enable UAM for DIA hours after bolus, or if a low temptarget is set
    var enableUAM=(bolusiob > 0.1 || (profile.temptargetSet && target_bg < 100) );
    ...
    // set meal_carbimpact high enough to absorb all meal carbs over 4 hours
    // total_impact (mg/dL) = CSF (mg/dL/g) * carbs (g)
    //console.error(csf * meal_data.carbs);
    // meal_carbimpact (mg/dL/5m) = CSF (mg/dL/g) * carbs (g) / 4 (h) * (1h/60m) * 5 (m/5m) * 2 (for linear decay)
    var meal_carbimpact = round((csf * meal_data.carbs / 4 / 60 * 5 * 2),1)
    console.error(meal_carbimpact,profile.min_5m_carbimpact,ci);
    ...
    if (meal_data.mealCOB * 3 > meal_data.carbs) {
        ...
        // set ci high enough to absorb all meal carbs over 4 hours
        ci = Math.max(profile.min_5m_carbimpact, meal_carbimpact, ci);
    }
    ...
    cid = meal_data.mealCOB * csf / ci;
    acid = meal_data.mealCOB * csf / aci;
    // duration (hours) = duration (5m) * 5 / 60 * 2 (to account for linear decay)
    console.error("Carb Impact:",ci,"mg/dL per 5m; CI Duration:",round(cid*5/60*2,1),"hours");
    ...
    try {
        iobArray.forEach(function(iobTick) {
            ...
            if ( cid && COBpredBG > maxPredBG ) { maxCOBPredBG = COBpredBG; }
            if ( enableUAM && UAMpredBGs.length > 9 && (UAMpredBG < minUAMPredBG) ) { minUAMPredBG = UAMpredBG; }
            if ( enableUAM && UAMpredBG > maxPredBG ) { maxUAMPredBG = UAMpredBG; }
    ...
    if (ci > 0) {
        ...
        // and if either maxPredBG is lower, use that
        minPredBG = Math.min(minPredBG, maxUAMPredBG, maxCOBPredBG);

    ...
    if (meal_data.mealCOB > 0 && !profile.enableautoisf_with_COB) {
        ...
        if (bg_ISF < 1) {                                                                                                      // mod V14j
            ...
            if ( liftISF < profile.autoisf_min ) {                                                                               // mod V14j
                isfadaptionreason = "final ISF factor " + round(liftISF,2) + " limitLoged by autoisf_min " + profile.autoisf_min;  // mod V14j
                console.error(isfadaptionreason);
                liftISF = profile.autoisf_min;                                                                      // mod V14j
            //} else if ( bg_ISF > maxISFReduction ) {                                                              // mod V14j: not possible here
            //    console.error("bg_ISF adaptation", round(bg_ISF,2), "limitLoged by autoisf_max", maxISFReduction);   // mod V14j
...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile) {
        ...
        if (pumphistory[j]._type == "TempBasal" && pumphistory[j].rate > 0) {
            ...
            do {
                ...
                if (j >= 0) {
                    ...
                    break;
                } else if (pumphistory[j]._type == "TempBasal" || pumphistory[j]._type == "PumpSuspend" || pumphistory[j]._type == "Rewind") {
    ...
    for (let k = 0; k < pumphistory.length; k++) {
        ...
        insulin = 0;
        if (pumphistory[k]['duration (min)'] == 0 || pumphistory[k]._type == "PumpResume" || pumphistory[k]._type == "Prime") {
    ...
    for (let n = pumphistory.length -1; n > 0; n--) {
        if (pumphistory[n]._type == "TempBasalDuration") {
            ...
            do {
                --o;
                if (o >= 0) {
                    if (pumphistory[o]._type == "TempBasal" || pumphistory[o]._type == "PumpSuspend" || pumphistory[o]._type == "Rewind") {
...
// Initialize rT (requestedTemp) object. Has to be done after eventualBG is calculated.
    rT = {
        ...
        , 'TDD': tdd // Total daily dose of insulin (past 24 hours) )
    };
...
// Begin core dosing logic: check for situations requiring low or high temps, and return appropriate temp after first match
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            //    console.error("profile.maxSMBBasalMinutes undefined: defaulting to 30m");
            // if IOB covers more than COB, limitLog maxBolus to 30m of basal

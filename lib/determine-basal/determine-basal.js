...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data) {
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && minZTGuardBG > target_bg && deviation > 0) {
            process.stderr.write("insulinReq adjusted from " + insulinReq);
            insulinReq = Math.max(insulinReq, (minZTGuardBG - target_bg) / sens);
            console.error("to",insulinReq,"for minZTGuardBG",minZTGuardBG);
        }

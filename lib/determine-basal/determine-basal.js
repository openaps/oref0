...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, offline, meal_data, setTempBasal) {
    ...
    //console.log("BG: "+bg+", minDelta: "+minDelta);
        //console.log( typeof(( bg > 180 && minDelta > Math.max(0,bgi) )));
        //console.log(( bg > 180 && minDelta > Math.max(0,bgi) ));
    var wtfAssist=false;
    // minDelta is > 7 and devation is > 25 wtf-assist
    if ( deviation > 25 && minDelta > 7 ) {
        wtfAssist=true;
    }
    // if BG > 180 and rising, wtf-assist
    // TODO: figure out why uncommenting this next line makes unrelated tests fail randomly
    if ( bg > 180 && minDelta > Math.max(0,bgi) ) {
        wtfAssist=true;
    }
    var mealAssist=false;
    if (
        ...
        (meal_data.carbs > 0 && (1.1 * meal_data.carbs/profile.carb_ratio > meal_data.boluses + basaliob))
        ) {
        mealAssist=true;
    }
    // allow meal assist to run when rising with carbs or rising fast for no good reason
    //console.log(mealAssist, wtfAssist);
    if (wtfAssist === true) { 
      //console.log("wtf");
    }
    //if (mealAssist === true || wtfAssist === true) {
    if (mealAssist === true) {
      //console.log("meal");

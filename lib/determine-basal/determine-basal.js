...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data) {
    ...
    var enableUAM=(bolusiob > 0 || (profile.temptargetSet && target_bg < 100) );
    ...
    try {
        iobArray.forEach(function(iobTick) {
            ...
            predUCIslope = Math.max(0, ci + ( COBpredBGs.length*minDeviationSlope ) );
            // if minDeviationSlope is too flat, predicted deviation impact drops linearly
            // from current deviation down to zero over DIA/2 (data points every 5m)
            predUCIdia = Math.max(0, ci * ( 1 - UAMpredBGs.length/Math.max(profile.dia/2*60/5,1) ) );
            //console.error(predUCIslope, predUCIdia);
            predUCI = Math.min(predUCIslope, predUCIdia);
            //console.error(COBpredBGs.length,minDeviationSlope, predUCI);
    ...
    IOBpredBG=round(IOBpredBGs[IOBpredBGs.length-1]);
    COBpredBG=round(COBpredBGs[COBpredBGs.length-1]);
    UAMpredBG=round(UAMpredBGs[UAMpredBGs.length-1]);

    rT.COB=meal_data.mealCOB;
    rT.IOB=iob_data.iob;
    rT.reason="COB: " + meal_data.mealCOB + ", Dev: " + deviation + ", BGI: " + bgi + ", ISF: " + convert_bg(sens, profile) + ", Target: " + convert_bg(target_bg, profile) + ", IOBpredBG: " + convert_bg(IOBpredBG, profile) + ", COBpredBG: " + convert_bg(COBpredBG, profile) + ", UAMpredBG: " + convert_bg(UAMpredBG, profile) + "; ";

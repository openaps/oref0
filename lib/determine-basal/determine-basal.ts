...
function calculate_expected_delta(target_bg: number, eventual_bg: number, bgi: number) {
...
function convert_bg(value: number, profile: Profile) {
...
function enable_smb(
    ...
    if (profile.enableSMB_always === true) {
        if (meal_data.bwFound) {
            console.error(
                'Warning: SMB enabled within 6h of using Bolus Wizard: be sure to easy bolus 30s before using Bolus Wizard'
            )
        } else {
    ...
    if (profile.enableSMB_with_COB === true && meal_data.mealCOB) {
        if (meal_data.bwCarbs) {
            console.error(
                'Warning: SMB enabled with Bolus Wizard carbs: be sure to easy bolus 30s before using Bolus Wizard'
            )
        } else {
    ...
    if (profile.enableSMB_high_bg === true && high_bg && bg >= high_bg) {
        console.error('Checking BG to see if High for SMB enablement.')
        console.error('Current BG', bg, ' | High BG ', high_bg)
        if (meal_data.bwFound) {
            console.error(
                'Warning: High BG SMB enabled within 6h of using Bolus Wizard: be sure to easy bolus 30s before using Bolus Wizard'
            )
        } else {
...
export const determine_basal = function determine_basal(
    ...
    const tick = glucose_status.delta > -0.5 ? `+${round(glucose_status.delta, 0)}` : round(glucose_status.delta, 0)
    ...
    if (minAgo > 12 || minAgo < -5) {
        ...
        rT.reason = `If current system time ${
            systemTime
        } is correct, then BG data is too old. The last BG data was read ${minAgo}m ago at ${bgTime}`
        // if BG is too old/noisy, or is changing less than 1 mg/dL/5m for 45m, cancel any high temps and shorten any long zero temps
    } else if (tooflat) {
        if (glucose_status.last_cal && glucose_status.last_cal < 3) {
            rT.reason = 'CGM was just calibrated'
        } else {
            rT.reason = `CGM data is unchanged (${bg}+${glucose_status.delta}) for 5m w/ ${
                glucose_status.short_avgdelta
            } mg/dL ~15m change & ${glucose_status.long_avgdelta} mg/dL ~45m change`
        }
    }
    ...
    if (profile.temptargetSet) {
        ...
        if (
            (profile.sensitivity_raises_target && autosens_data.ratio < 1) ||
    ...
    if (glucose_status.noise >= 2) {
        ...
        const adjustedMinBG = round(Math.min(200, min_bg * noisyCGMTargetMultiplier))
        const adjustedTargetBG = round(Math.min(200, target_bg * noisyCGMTargetMultiplier))
        const adjustedMaxBG = round(Math.min(200, max_bg * noisyCGMTargetMultiplier))
    ...
    const rTDeliveredAt = deliverAt
    rT.temp = 'absolute'
    rT.deliverAt = rTDeliveredAt
    if (
        microBolusAllowed &&
        currenttemp &&
        iob_data.lastTemp &&
    ...
    if (currenttemp && iob_data.lastTemp && currenttemp.duration > 0) {
        ...
        if (lastTempEnded > 5 && lastTempAge > 10) {
            rT.reason = `Warning: currenttemp running but lastTemp from pumphistory ended ${
                lastTempEnded
    ...
    if (iob_data.iob > 0) {
        ...
        // if IOB is negative, be more conservative and use the lower of sens, profile.sens
    ...
    const COBpredBGs: number[] = []
    const IOBpredBGs: number[] = []
    const UAMpredBGs: number[] = []
    const ZTpredBGs: number[] = []
    COBpredBGs.push(bg)
    IOBpredBGs.push(bg)
    ZTpredBGs.push(bg)
    UAMpredBGs.push(bg)

    let enableSMB = enable_smb(profile, microBolusAllowed, meal_data, bg, target_bg, high_bg)
    ...
    let remainingCATimeMin = 3 // h; minimum duration of expected not-yet-observed carb absorption
    ...
    if (meal_data.carbs) {
        ...
        console.error(
            'Last carbs',
            lastCarbAge,
            'minutes ago; remainingCATime:',
            remainingCATime,
            'hours;',
            `${round(fractionCOBAbsorbed * 100)}% carbs absorbed`
        )
    }
    ...
    console.error(
        'Carb Impact:',
        ci,
        'mg/dL per 5m; CI Duration:',
        round(((cid * 5) / 60) * 2, 1),
        'hours; remaining CI (',
        remainingCATime,
        ' peak):',
        round(remainingCIpeak, 1),
        'mg/dL per 5m'
    )
    ...
    rT.predBGs = {}
    IOBpredBGs.forEach((p, i, theArray) => {
    ...
    if (meal_data.mealCOB > 0 && (ci > 0 || remainingCIpeak > 0)) {
        COBpredBGs.forEach((p, i, theArray) => {
            theArray[i] = round(Math.min(401, Math.max(39, p)))
        })
    ...
    if (ci > 0 || remainingCIpeak > 0) {
        if (enableUAM) {
            UAMpredBGs.forEach((p, i, theArray) => {
                theArray[i] = round(Math.min(401, Math.max(39, p)))
            })
    ...
    if (UAMpredBG && COBpredBG && minUAMPredBG < 999 && minCOBPredBG < 999) {
    ...
    rT.reason = `COB: ${rT.COB}, Dev: ${rT.deviation}, BGI: ${rT.BGI}, ISF: ${rT.ISF}, CR: ${
        rT.CR
    }, minPredBG: ${convert_bg(minPredBG, profile)}, minGuardBG: ${convert_bg(
        minGuardBG,
        profile
    )}, IOBpredBG: ${convert_bg(lastIOBpredBG, profile)}`
    if (lastCOBpredBG && lastCOBpredBG > 0) {
        rT.reason += `, COBpredBG: ${convert_bg(lastCOBpredBG, profile)}`
    }
    if (lastUAMpredBG && lastUAMpredBG > 0) {
    ...
    if (enableSMB && minGuardBG < threshold) {
        console.error(
            'minGuardBG',
            convert_bg(minGuardBG, profile),
            'projected below',
            convert_bg(threshold, profile),
            '- disabling SMB'
        )
    ...
    let maxDelta_bg_threshold = 0.2
    if (profile.maxDelta_bg_threshold !== undefined) {
        maxDelta_bg_threshold = Math.min(profile.maxDelta_bg_threshold, 0.3)
    }
    if (maxDelta > maxDelta_bg_threshold * bg) {
        console.error(
            `maxDelta ${convert_bg(maxDelta, profile)} > ${100 * maxDelta_bg_threshold}% of BG ${convert_bg(bg, profile)} - disabling SMB`
        )
        rT.reason += `maxDelta ${convert_bg(maxDelta, profile)} > ${100 * maxDelta_bg_threshold}% of BG ${convert_bg(
            bg,
            profile
        )}: SMB disabled; `
        enableSMB = false
    ...
    if (
        bg < threshold &&
        iob_data.iob < (-profile.current_basal * 20) / 60 &&
        minDelta > 0 &&
        minDelta > expectedDelta
    ) {
        rT.reason += `IOB ${iob_data.iob} < ${round((-profile.current_basal * 20) / 60, 2)}`
        rT.reason +=
            ` and minDelta ${convert_bg(minDelta, profile)} > ` +
            `expectedDelta ${convert_bg(expectedDelta, profile)}; `
    ...
    if (eventualBG < min_bg) {
        // if eventual BG is below target:
    ...
    if (minDelta < expectedDelta) {
        ...
        if (!(microBolusAllowed && enableSMB)) {
            if (glucose_status.delta < minDelta) {
                rT.reason += `Eventual BG ${convert_bg(eventualBG, profile)} > ${convert_bg(
                    min_bg,
                    profile
                )} but Delta ${convert_bg(Number(tick), profile)} < Exp. Delta ${convert_bg(expectedDelta, profile)}`
            } else {
                rT.reason += `Eventual BG ${convert_bg(eventualBG, profile)} > ${convert_bg(
                    min_bg,
                    profile
                )} but Min. Delta ${minDelta.toFixed(2)} < Exp. Delta ${convert_bg(expectedDelta, profile)}`
            }
            if (currenttemp.duration > 15 && round_basal(basal, profile) === round_basal(currenttemp.rate, profile)) {
                rT.reason += `, temp ${currenttemp.rate} ~ req ${basal}U/hr. `
                return rT
    ...
    if (max_iob !== undefined && iob_data.iob > max_iob) {
        ...
        if (max_iob !== undefined && insulinReq > max_iob - iob_data.iob) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            console.error(
                `naive_eventualBG ${convert_bg(naive_eventualBG, profile)}, ${durationReq}m ${smbLowTempReq}U/h temp needed; last bolus ${lastBolusAge}m ago; maxBolus: ${maxBolus}`
            )

...
var getLastGlucose = function (data) {
                    ...
                    if (D != 0) {
                        ...
                        if (n > 3) {
                            if (r_squ > corrMax) {
                                ...
                                delta_pl =-scaleBg * (a * Math.pow(-5 * 60 / scaleTime, 2) - b * 5 * 60 / scaleTime);     // 5 minute slope from last fitted bg starting from last bg, i.e. t=0
                                delta_pn = scaleBg * (a * Math.pow( 5 * 60 / scaleTime, 2) + b * 5 * 60 / scaleTime);     // 5 minute slope to next fitted bg starting from last bg, i.e. t=0
    ...
    return {
        ...
        , parabola_fit_a0: Math.round( a_0 * 100) / 100
        , parabola_fit_a1: Math.round( a_1 * 100) / 100
        , parabola_fit_a2: Math.round( a_2 * 100) / 100
        , pp_debug
        // end autoISF values
        , date: now_date
        , last_cal: last_cal

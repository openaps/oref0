...
var getLastGlucose = function (data) {
    ...
    //  mod 7c: stop the series if there was a CGM gap greater than 13 minutes, i.e. 2 regular readings
            ...
            if (data.length <= 3) {                      // last 3 points make a trivial parabola
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (D != 0) {
                        ...
                        for (var j = 0; j <= i; j++) {
                            ...
                            s_squares += Math.pow(before.glucose - y_mean, 2);
                            var delta_t = (before_date - time_0) / 1000;
                            s_residual_squares += Math.pow(before.glucose - a * Math.pow(delta_t, 2) - b * delta_t - c, 2);
                        }
                        var r_squ = 0.64;
                        if (s_squares != 0) {
                        ...
                        if (n > 3) {
                            if (r_squ > corrMax) {
                                ...
                                dura_p = -ti / 60;            // remember we are going backwards in time
                ...
                pp_debug += "coeffs=(" + round(best_a,2) + " / " + round(best_b,2) + " / " + round(best_c,2) + "); bg date=" + time_0 + "; ";
            }
           pp_debug += "Slopes=(" + round(slope05,2) + " / " + round(slope15,2) + " / " + round(slope40,2) + "); "
    return {

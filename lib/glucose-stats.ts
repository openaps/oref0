...
export function calcSensorNoise(calcGlucose: any, glucoseHist: any, lastCal: any, sgv: any) {
    const MAXRECORDS = 8
    const MINRECORDS = 4
    const sgvArr = []
    ...
    for (let i = numRecords; i < glucoseHist.length; i += 1) {
        ...
        if (
            lastCal &&
            glucoseHist[i].glucose > 30 &&
            'unfiltered' in glucoseHist[i] &&
            glucoseHist[i].unfiltered > 100
        ) {
...
export function calcTrend(calcGlucose: any, glucoseHist: any, lastCal: any, sgv: any) {
    let sgvHist = null
    ...
    if (glucoseHist.length > 0) {
        ...
        let timeSpan = 0
        let totalDelta = 0
        const currentTime = new Date(sgv ? sgv.readDateMills : glucoseHist[glucoseHist.length - 1].readDateMills)

        sgvHist = []

        // delete any deltas > 16 minutes and any that don't have an unfiltered value (backfill records)
        let minDate = currentTime.getTime() - 16 * 60 * 1000
        for (let i = 0; i < glucoseHist.length; i += 1) {
            if (
                lastCal &&
                glucoseHist[i].readDateMills >= minDate &&
                'unfiltered' in glucoseHist[i] &&
                glucoseHist[i].unfiltered > 100
            ) {
                sgvHist.push({
        ...
        console.error(`Not enough history for trend calculation: ${glucoseHist.length}`)
    }
...
// Return sensor noise
export function calcNSNoise(noise: number, glucoseHist: any) {
    ...
    if (!currSGV) {
        ...
        console.error(`Glucose ${currSGV.glucose} > 400 - setting noise level Heavy`)
        nsNoise = 4
    } else if (currSGV.glucose < 40) {
        console.error(`Glucose ${currSGV.glucose} < 40 - setting noise level Light`)
        nsNoise = 2
        ...
        console.error(`Glucose change ${deltaSGV} out of range [-30, 30] - setting noise level Heavy`)
        nsNoise = 4
    } else if (noise < 0.35) {
        nsNoise = 1 // Clean
    } else if (noise < 0.5) {
...
export function NSNoiseString(nsNoise: any) {
    switch (nsNoise) {
        case 1:
            return 'Clean'
        case 2:

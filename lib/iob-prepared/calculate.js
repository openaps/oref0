...
function iobCalc(treatment, time, dia) {
    ...
    if (treatmentStartTime.getTime() === treatmentEndTime.getTime()) {
      if (treatment.unit === "U") {
      ...
      var rate = 0;
      if (treatment.unit === "U") {
        // in the unlikely case that start time and end time are different
        // convert to rate in U/hour
        var durationHours = (treatmentEndTime - treatmentStartTime) / 1000 / 60 / 60;
        rate = treatment.amount / durationHours;
      }
      else if (treatment.unit === 'U/hour') {
        rate = treatment.amount;
      }
      iobContrib = dia * rate * (f2(x_end) - f2(x_start));
      activityContrib = dia * rate * (f1(x_start) - f1(x_end));
    }

    ...
    if ( profile.curve == "rapid-acting" ) {
        if (profile.useCustomPeakTime && profile.insulinPeakTime !== undefined) {
            if ( profile.insulinPeakTime > 120 ) {
                console.error('Setting maximum Insulin Peak Time of 120m for',curve,'insulin');
                peak = 120;
            } else if ( profile.insulinPeakTime < 50 ) {
                console.error('Setting minimum Insulin Peak Time of 50m for',curve,'insulin');
                peak = 50;
            } else {
        ...
        if (profile.useCustomPeakTime && profile.insulinPeakTime !== undefined) {
            if ( profile.insulinPeakTime > 100 ) {
                console.error('Setting maximum Insulin Peak Time of 100m for',curve,'insulin');
                peak = 100;
            } else if ( profile.insulinPeakTime < 35 ) {
                console.error('Setting minimum Insulin Peak Time of 35m for',curve,'insulin');
                peak = 35;
            } else {
                peak = profile.insulinPeakTime;
            }

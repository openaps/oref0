...
function iobCalc(treatment, time, dia, profile) {
    ...
    if (!(curve in curveDefaults)) {
        console.log('Unsupported curve function: "' + curve + '". Supported curves: "bilinear", "rapid-acting" (Novolog, Novorapid, Humalog, Apidra) and "ultra-rapid" (Fiasp). Defaulting to "rapid-acting".');
        curve = 'bilinear';
    }
    ...
    if (defaults.requireLongDia && dia < 5) {
        console.log('Pump DIA must be set to 5 hours or more with the new curves, please adjust your pump. Defaulting to 5 hour DIA.');
        td = 300;
    }

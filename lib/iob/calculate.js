...
function iobCalc(treatment, time, dia, profile) {
    ...
    if (curve != 'fast-acting' && curve != 'ultrafast') {
        console.error('Unsupported curve function: "' + curve + '". Supported curves: bilinear, fast-acting (Novolog, Novorapid, Humalog, Apidra) and ultrafast (Fiasp).');
        return;
    }
    ...
    if (profile.insulinPeakTime !== undefined && curve == 'ultrafast') {
        diaratio = 55.0 / Number(profile.insulinPeakTime);
        AUCratio = diaratio;
    }

    if (profile.insulinPeakTime !== undefined && curve == 'fast-acting') {
    ...
    if (treatment.insulin) {
        ...
        if (minAgo < end) {
            if (curve.toLowerCase() === 'ultrafast') {

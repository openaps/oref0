...
import type { BolusTreatment, InsulinTreatment } from './InsulinTreatment'
...
export function calculate(
    ...
    if (isBolusTreatment(treatment)) {
        time = time || new Date()
        const bolusTime = new Date(treatment.date)
        const minsAgo = Math.round((time.getTime() - bolusTime.getTime()) / 1000 / 60)
...
function iobCalcBilinear(treatment: BolusTreatment, minsAgo: number, dia: number) {
    ...
    const timeScalar = default_dia / dia
    const scaled_minsAgo = timeScalar * minsAgo
    ...
    const activityPeak = 2 / (dia * 60)
    const slopeUp = activityPeak / peak
    const slopeDown = -1 * (activityPeak / (end - peak))

    if (scaled_minsAgo < peak) {
        activityContrib = treatment.insulin * (slopeUp * scaled_minsAgo)

        const x1 = scaled_minsAgo / 5 + 1 // scaled minutes since bolus, pre-peak; divided by 5 to work with coefficients estimated based on 5 minute increments
        iobContrib = treatment.insulin * (-0.001852 * x1 * x1 + 0.001852 * x1 + 1.0)
    } else if (scaled_minsAgo < end) {
        const minsPastPeak = scaled_minsAgo - peak
        activityContrib = treatment.insulin * (activityPeak + slopeDown * minsPastPeak)

        const x2 = (scaled_minsAgo - peak) / 5 // scaled minutes past peak; divided by 5 to work with coefficients estimated based on 5 minute increments
        iobContrib = treatment.insulin * (0.001323 * x2 * x2 + -0.054233 * x2 + 0.55556)
    }
...
function iobCalcExponential(treatment: BolusTreatment, minsAgo: number, dia: number, peak: number, profile: Profile) {
    ...
    const end = dia * 60 // end of insulin activity, in minutes
    ...
    if (minsAgo < end) {
        ...
        activityContrib =
            treatment.insulin * (S / Math.pow(tau, 2)) * minsAgo * (1 - minsAgo / end) * Math.exp(-minsAgo / tau)
        iobContrib =
            treatment.insulin *
            (1 -
                S *
                    (1 - a) *
                    ((Math.pow(minsAgo, 2) / (tau * end * (1 - a)) - minsAgo / tau - 1) * Math.exp(-minsAgo / tau) + 1))

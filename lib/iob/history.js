    ...
    for (var i=0; i < tempHistory.length; i++) {
        if (tempHistory[i].duration > 0) {
            ...
            if (!_.isEmpty(profile_data.basalprofile)) {
            	currentRate = basalprofile.basalLookup(profile_data.basalprofile,new Date(tempHistory[i].timestamp));
            }
                        
            var netBasalRate = tempHistory[i].rate - currentRate; //profile_data.current_basal;            

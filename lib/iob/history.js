...
function splitTimespanWithOneSpliter(event,splitter) {
	...
	if (splitter.type == 'recurring') {
		...
		if ((startMinutes < splitter.minutes && endMinutes > splitter.minutes) || (endMinutes > 1440 && splitter.minutes < (endMinutes - 1440))) {
			...
			var divisorPoint = splitter.minutes;
			if (endMinutes > 1440) { divisorPoint = endMinutes - 1440 + splitter.minutes; }

			event1EndDate = moment(event.started_at).add(divisorPoint,'minutes');
			event1Duration = event1EndDate.diff(event.started_at,'minutes');
	...
	while(splits > 0) {
		...
		_.forEach(results,function split(o) {
			...
			_.forEach(splitterMoments,function split(p) {
				var splitResult = splitTimespanWithOneSpliter(o,p);
		...
		splits = resultArray.length - results.length;
		results = resultArray;
	}
...
function calcTempTreatments (inputs) {
  ...
  var profile_data = inputs.profile;
    ...
    // Split events to match basal profile
    ...
    //console.log("splitterEvents",splitterEvents);

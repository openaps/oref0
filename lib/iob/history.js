...
function splitTimespanWithOneSplitter(event,splitter) {
	...
	if (splitter.type === 'recurring') {
		...
		if ((startMinutes < splitter.minutes && endMinutes > splitter.minutes) || (endMinutes > 1440 && splitter.minutes < (endMinutes - 1440))) {
			...
			if (endMinutes > 1440) { splitPoint = endMinutes - 1440 + splitter.minutes; }

			var event1EndDate = moment(event.started_at).add(splitPoint,'minutes');
			var event1Duration = event1EndDate.diff(event.started_at,'minutes');

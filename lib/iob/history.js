...
function calcTempTreatments (inputs, zeroTempDuration) {
    ...
    // disable for now until updated code achieves identical results as before
    if (true) {
        // split the current item up around any pump suspends
        //
        var splitHistory = [];

        _.forEach(splitHistoryByBasal, function splitSuspendEvent(o, firstResumeTime, suspendedPrior, lastSuspendTime, currentlySuspended) {
            splitHistory = splitHistory.concat(splitAroundSuspends(o, firstResumeTime, suspendedPrior, lastSuspendTime, currentlySuspended));
        });
    } else {

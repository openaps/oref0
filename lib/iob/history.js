...
function calcTempTreatments (inputs, zeroTempDuration) {
    ...
    for (var i=0; i < pumpHistory.length; i++) {
		...
		while (iter < pumpHistory.length && pumpHistory[iter].timestamp == timestamp) {
			if (pumpHistory[iter]._type == "TempBasalDuration") {
		...
		{
			console.error("No duration found for "+rate+" U/hr basal"+date, pumpHistory[i - 1], current, pumpHistory[i + 1]); 
		}
	    }
            var temp = {};
            temp.rate = rate;

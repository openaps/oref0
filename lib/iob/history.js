...
function splitTimespanWithOneSplitter(event,splitter) {
	...
	if (splitter.type === 'recurring') {
		...
		// 1440 magic for checking if the event overlaps midnight

		if (event.duration > 15 || (startMinutes < splitter.minutes && endMinutes > splitter.minutes) || (endMinutes > 1440 && splitter.minutes < (endMinutes - 1440))) {

//			console.log('SPLITTING',startMinutes, endMinutes,splitter.minutes);
...
/*			var startMinutes_new = event.started_at.getHours() * 60 + event.started_at.getMinutes();
			var endMinutes_new = startMinutes_new + event.duration;

			var startMinutes_new2 = event2.started_at.getHours() * 60 + event2.started_at.getMinutes();
			var endMinutes_new2 = startMinutes_new2 + event2.duration;

			console.log('new event 1',startMinutes_new, endMinutes_new, event1.duration);
			console.log('new event 2',startMinutes_new2, endMinutes_new2, event2.duration);
*/	
...
//	console.log('tempHistory after sorting:',tempHistory);
    
    for (var i=0; i+1 < tempHistory.length; i++) {
        if (tempHistory[i].date + tempHistory[i].duration*60*1000 > tempHistory[i+1].date) {
//        	console.log('Overlap! Early:',tempHistory[i],'later',tempHistory[i+1]);
//        	console.log('Adjusting event length from',tempHistory[i].duration,'to',(tempHistory[i+1].date - tempHistory[i].date)/60/1000);
...
/*	if (tempHistory.length != splitHistory.length)
	{

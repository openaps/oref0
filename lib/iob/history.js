...
function calcTempTreatments (inputs, zeroTempDuration) {
    ...
    var firstResumeDate, lastSuspendDate;
    ...
    pumpSuspends = _.sortBy(pumpSuspends, function(o) { return o.date; });

    pumpResumes = _.sortBy(pumpResumes, function(o) { return o.date; });
    ...
    if (pumpResumes[0].date < pumpSuspends[0].date) {
        suspendFirst = true;
        firstResumeDate = pumpResumes[0].date;
    }

    // Check to see if the last suspend is after the last resume
    // indicating the last suspend was still in effect at the end
    // of this block of history
    if (pumpResumes[pumpResumes.length-1].date < pumpSuspends[pumpSuspends.length-1].date) {
        resumeLast = true;
        lastSuspendDate = pumpSuspends[pumpSuspends.length-1].date;
    }

    if (!suspendFirst && !resumeLast && (pumpResumes.length != pumpSuspends.length)) {
        console.error("Mismatched number of resumes("+pumpResumes.length+") and suspends("+pumpSuspends.length+")!");
        suspendError = true;
    } else if (suspendFirst && !resumeLast && ((pumpResumes.length-1) != pumpSuspends.length)) {
        ...
        suspendError = true;
    } else if (!suspendFirst && resumeLast && (pumpResumes.length != (pumpSuspends.length-1))) {
        ...
        suspendError = true;
    } else if (suspendFirst && resumeLast && (pumpResumes.length != pumpSuspends.length)) {
    ...
    // Match the resumes with the suspends
    // JPC - check to make sure next suspend comes after the matching resume
    // JPC - create tempHistory entries with 0 insulin rates, will need entry types set so code checking for overlapping entries can properly adjudicate
    //       the higher priority suspend events
    for (var i=0; i < pumpSuspends.length; i++) {
        if (!suspendFirst) {
            pumpSuspends[i].duration = (pumpResumes[i].date - pumpSuspends[i].date)/60/1000;
        } else {
            pumpSuspends[i].duration = (pumpResumes[i+1].date - pumpSuspends[i].date)/60/1000;
        }

        if (pumpSuspends[i].duration < 0) {
            console.error("Pump suspends and resumes out of sequence!");
            suspendError = true;
        }
    }

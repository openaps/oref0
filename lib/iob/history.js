    ...
    for (var i=0; i < pumpHistory.length; i++) {
        ...
        if (current._type == "Bolus") {
            ...
            tempBoluses.push(temp);
                // Versuch für Nightscout Careportal-Einträge
        } else if (current._type == "Meal Bolus" {
            var temp = {};
            temp.timestamp = current.created_at;
            temp-started_at = new Date(tz(current.created_at));
            temp.date = temp.created_at.getTime();

...
function calcTempTreatments (inputs, zeroTempDuration) {
    ...
    if(inputs.history24) {
        pumpHistory =  [ ].concat(inputs.history).concat(inputs.history24);
    }
    ...
    if (suspend_zeros_iob) {
        ...
        if (currentlySuspended) {
            suspendStart = new Date(lastSuspendTime);
            suspendStartDate = suspendStart.getTime()
            started_at = new Date(tz(suspendStart.toISOString()));
    ...
    for (i=0; i < splitHistory.length; i++) {
        ...
        if (currentItem.duration > 0) {
            ...
            if (typeof profile_data.min_bg !== 'undefined' && typeof profile_data.max_bg !== 'undefined') {
                var target_bg = (profile_data.min_bg + profile_data.max_bg) / 2;
            }

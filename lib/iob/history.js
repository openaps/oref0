    ...
    for (var i=0; i < tempHistory.length; i++) {
        if (tempHistory[i].duration > 0) {
            ...
            if (_.isEmpty(profile_data.basalprofile)) {
            	...
            	rate = basalprofile.basalLookup(profile_data.basalprofile,new Date(tempHistory[i].timestamp));
            }
                        
            var netBasalRate = tempHistory[i].rate - rate; //profile_data.current_basal;            
            ...
            //console.error("Temp basal rate: ", tempHistory[i].rate, ", programmed basal rate: ", rate, " netBasalRate: ", netBasalRate, " netBasalAmount: ", netBasalAmount);

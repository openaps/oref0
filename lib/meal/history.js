    ...
    for (var i=0; i < pumpHistory.length; i++) {
        ...
        if (current._type == "Bolus" && current.timestamp) {
            ...
            if (!arrayHasElementWithSameTimestampAndProperty(mealInputs,current.timestamp,"carbs")) {
                if (arrayHasElementWithSameTimestampAndProperty(pumpHistory,current.timestamp,"amount") || arrayHasElementWithSameTimestampAndProperty(mealInputs,current.timestamp,"amount")) {
                    mealInputs.push(temp);
                    //bwCarbs += temp.carbs;
                } else {

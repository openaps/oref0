...
var calcMealCOB = require('../determine-basal/cob-autosens');

function diaCarbs(opts, time) {
    var treatments = opts.treatments;
    var profile_data = opts.profile;

    var glucose_data = null;

    // TODO: this check is actually invalid, because later calcMealCOB call fails if no glucose data is provided
    if (typeof(opts.glucose) !== 'undefined') {
        glucose_data = opts.glucose;
    }
    var carbs = 0;
    var boluses = 0;

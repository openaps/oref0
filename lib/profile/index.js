...
function defaults ( ) {
  return /* profile */ {
    ...
    , autoisf: false // enable autoisf code default is false
    , autoisf_hourlychange: 0.25 // rate at which autoISF grows per hour assuming bg is twice target. When value = 1.0, ISF is reduced to 50% after 1 hour of BG at 2x target
    , autoisf_max: 1.3 // Multiplier cap on how high the autoISF ratio can be and therefore how low it can adjust ISF
    , smb_delivery_ratio: 0.6 //Default value: 0.5 This is another key OpenAPS safety cap, and specifies what share of the total insulin required can be delivered as SMB. This is to prevent people from getting into dangerous territory by setting SMB requests from the caregivers phone at the same time. Increase this experimental value slowly and with caution.
    , smb_max_range_extension: 2 //Default value: 1 This is another key OpenAPS safety cap, and specifies by what factor you can exceed the regular 120 maxSMB/maxUAM minutes. Increase this experimental value slowly and with caution.
...
function displayedDefaults () {
    ...
    profile.use_autoisf = allDefaults.autoisf;

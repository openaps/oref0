...
function lookup (inputs) {
    ...
    for (var i = 0; i < temptargets_data.length; i++) {
        ...
        if (now >= new Date(temptargets_data[i].created_at) && now < expires ) {
            //console.error(temptargets_data[i]);
            bgTargets.high = temptargets_data[i].targetTop;
            bgTargets.low = temptargets_data[i].targetBottom;
            //break;
        }
    }
...
function bound_target_range (target) {
    ...
    target.max_bg = Math.min(200, target.min_bg);
    return target
}

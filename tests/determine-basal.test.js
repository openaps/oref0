...
describe('determine-basal', function ( ) {
    ...
    var mealAssistFn = require('../lib/determine-basal/meal-assist');
   ...
   //function determine_basal(glucose_status, currenttemp, iob_data, profile)
    ...
    it('should cancel any temp when in range w/o IOB', function () {
        ...
        var output = determine_basal(glucose_status, currenttemp, iob_data, profile, undefined, meal_data, mealAssistFn, setTempBasal);
    ...
    it('should bg be < 30 (Dexcom is in ???) return error', function () {
      var result = determine_basal({glucose:18},undefined, undefined, {"current_basal":0.0}, undefined, meal_data, mealAssistFn, setTempBasal);
      result.error.should.equal('CGM is calibrating or in ??? state');
    });  

    it('profile should contain min_bg,max_bg or target_bg', function () {
      var result = determine_basal({glucose:100},undefined, undefined, {"current_basal":0.0}, undefined, meal_data, mealAssistFn, setTempBasal);
      result.error.should.equal('Error: could not determine target_bg');
    }); 

    it('iob_data should not be undefined', function () {
      var result = determine_basal({glucose:100},undefined, undefined, {"current_basal":0.0, "target_bg":100}, undefined, meal_data, mealAssistFn, setTempBasal);
      result.error.should.equal('Error: iob_data undefined');
    }); 

    it('iob_data should contain activity, iob, bolussnooze', function () {
      var result = determine_basal({glucose:100}, undefined,{"activity":0}, {"current_basal":0.0, "target_bg":100}, undefined, meal_data, mealAssistFn, setTempBasal);
...
/*
    it('should return error eventualBG if something went wrong', function () {
      var result = determine_basal({glucose:100}, undefined,{"activity":0, "iob":0,"bolussnooze":0}, {"current_basal":0.0, "target_bg":100, "sens":NaN}, undefined, meal_data, mealAssistFn, setTempBasal);
...
*/
    ...
    it('should temp to zero with double sensitivity adjustment', function () {
        ...
        var output = determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, mealAssistFn, setTempBasal);
        //console.log(output);
        output.rate.should.equal(0);
        output.duration.should.equal(30);
    });

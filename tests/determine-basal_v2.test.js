...
describe('round_basal', function () {
    ...
    it('V2 should round correctly without profile being passed in', function () {
        var basal = 0.025;
    ...
    it('V2 should round correctly with an old pump model', function () {
        var basal = 0.025;
    ...
    it('V2 should round correctly with a new pump model', function () {
        var basal = 0.025;
        profile.model = "554";
        var output = round_basal(basal, profile);
    ...
    it('V2 should round correctly with an invalid pump model', function () {
        var basal = 0.025;
    ...
    data.forEach(function (rate) {
        it('V2 should round basal rates properly (' + rate.basal + ' -> ' + rate.rounded + ')', function () {
...
function getIOBForTest(recentDose, profile) {
    ...
    return generate({
        history: treatments,
        history24: treatments,
        profile: profile,
        clock: now.toISOString()
    });
}

function getNegativeIOB(desiredIOB, profile) {
    ...
    const multiplier = Math.abs(desiredIOB) / p.basalprofile[0].rate;
    const duration = multiplier * 60;
    const now = new Date();
    const past = new Date(now.getTime() - duration * 60 * 1000);

    console.log('getNegativeIOB desired iob', desiredIOB,'duration',duration);
    ...
    console.log('getNegativeIOB reporting', rVal);
...
describe('determine-basal', function () {
    ...
    console.log('IOB0', iob_0);

    it('V2 should cancel high temp when in range w/o IOB', function () {
        var currenttemp = {
            "duration": 30,
            "rate": 1.5,
            "temp": "absolute"
        };
        var output = determine_basal(glucose_status, currenttemp, getIOBForTest(0, profile), profile, autosens, meal_data, tempBasalFunctions);
    ...
    //it('V2 should let low temp run in range w/o IOB', function () {
    ...
    it('V2 should temp to 0 when low w/o IOB', function () {
        var glucose_status = {
            "delta": -5,
            "glucose": 75,
            "long_avgdelta": -5,
    ...
    it('V2 should not extend temp to 0 when <10m elapsed', function () {
        var currenttemp = {
            "duration": 57,
            "rate": 0,
            "temp": "absolute"
    ...
    it('V2 should do nothing when low and rising w/o IOB', function () {
        var glucose_status = {
            "delta": 6,
            "glucose": 80,
            "long_avgdelta": 6,
    ...
    //it('V2 should do nothing when low and rising w/ negative IOB', function () {
    ...
    //it('V2 should do nothing on large uptick even if avgdelta is still negative', function () {
    ...
    it('V2 should temp to zero when rising slower than BGI', function () {
        var glucose_status = {
            "delta": 1,
            "glucose": 75,
            "long_avgdelta": 1,
        ...
        var output = determine_basal(glucose_status, currenttemp, getNegativeIOB(-0.5), profile, autosens, meal_data, tempBasalFunctions);
        console.log(output);
        output.rate.should.equal(0);
        output.duration.should.equal(30);
        //output.reason.should.match(/BG 75<80/);
    ...
    it('V2 should temp to 0 when low and falling, regardless of BGI', function () {
        var glucose_status = {
            "delta": -1,
            "glucose": 75,
            "long_avgdelta": -1,
        ...
        var output = determine_basal(glucose_status, currenttemp, getIOBForTest(1, profile), profile, autosens, meal_data, tempBasalFunctions);
        console.log(output);
        output.rate.should.equal(0);
        output.duration.should.be.above(29);
        //output.reason.should.match(/BG 75<80/);
    ...
    //it('V2 should cancel high-temp when low and rising faster than BGI', function () {
    ...
    //it('V2 should cancel low-temp when eventualBG is higher then max_bg', function () {
    ...
    it('V2 should high-temp when > 80-ish and rising w/ lots of negative IOB', function () {
        var glucose_status = {
            "delta": 5,
            "glucose": 85,
            "long_avgdelta": 5,
        ...
        var output = determine_basal(glucose_status, currenttemp, getNegativeIOB(-1), profile, autosens, meal_data, tempBasalFunctions);
        output.rate.should.be.above(1);
        output.duration.should.equal(30);
        output.reason.should.match(/no temp, setting/);
    });

    it('V2 should high-temp when > 180-ish and rising but not more then maxSafeBasal', function () {
        var glucose_status = {
            "delta": 5,
            "glucose": 185,
            "long_avgdelta": 5,
    ...
    it('V2 should reduce high-temp when schedule would be above max', function () {
        var glucose_status = {
            "delta": 5,
            "glucose": 145,
            "long_avgdelta": 5,
    ...
    it('V2 should continue high-temp when required ~= temp running', function () {
        var glucose_status = {
            "delta": 5,
            "glucose": 145,
            "long_avgdelta": 5,
    ...
    it('V2 should set high-temp when required running temp is low', function () {
        var glucose_status = {
            "delta": 5,
            "glucose": 145,
            "long_avgdelta": 5,
    ...
    it('V2 should stop high-temp when iob is near max_iob.', function () {
        var glucose_status = {
            "delta": 5,
            "glucose": 485,
            "long_avgdelta": 5,
        ...
        var output = determine_basal(glucose_status, currenttemp, getIOBForTest(3.5, profile), profile, autosens, meal_data, tempBasalFunctions);
        output.rate.should.equal(0.9);
        output.duration.should.equal(30);
        output.reason.should.match(/IOB .* > max_iob .*/);
    });

    it('V2 should temp to 0 when LOW w/ positive IOB', function () {
        var glucose_status = {
            "delta": 0,
            "glucose": 39,
            "long_avgdelta": -1.1,
    ...
    it('V2 should low temp when LOW w/ negative IOB', function () {
        var glucose_status = {
            "delta": 0,
            "glucose": 39,
            "long_avgdelta": -1.1,
        ...
        var output = determine_basal(glucose_status, currenttemp, getNegativeIOB(-2.5,profile), profile, autosens, meal_data, tempBasalFunctions);
        console.log(output);
    ...
    it('V2 should temp to 0 when LOW w/ no IOB', function () {
        var glucose_status = {
            "delta": 0,
            "glucose": 39,
            "long_avgdelta": -1.1,
    ...
    it('V2 should low-temp when eventualBG < min_bg', function () {
        var glucose_status = {
            "delta": -3,
            "glucose": 110,
            "long_avgdelta": -1,
        ...
        output.rate.should.be.below(0.8);
        output.duration.should.be.above(29);
        output.reason.should.match(/Eventual BG .*< 115.*/);
    });

    it('V2 should low-temp when eventualBG < min_bg with delta > exp. delta', function () {
        var glucose_status = {
            "delta": -5,
            "glucose": 115,
            "long_avgdelta": -6,
        ...
        var output = determine_basal(glucose_status, currenttemp, getIOBForTest(2, profile), profile, autosens, meal_data, tempBasalFunctions);
        console.log(output);
    ...
    it('V2 should low-temp when eventualBG < min_bg with delta > exp. delta', function () {
        ...
        var output = determine_basal(glucose_status, currenttemp, getIOBForTest(3.51, profile), profile, autosens, meal_data, tempBasalFunctions);
        console.log(output);
        output.rate.should.be.below(0.8);
        output.duration.should.equal(30);
        output.reason.should.match(/Eventual BG .*< 115.*setting .*/);
    });

    it('V2 should low-temp much less when eventualBG < min_bg with delta barely negative', function () {
        var glucose_status = {
            "delta": -1,
            "glucose": 115,
            "long_avgdelta": -1,
    ...
    //it('V2 should do nothing when eventualBG < min_bg but appropriate low temp in progress', function () {
    ...
    it('V2 should cancel low-temp when lowish and avg.delta rising faster than BGI', function () {
        var currenttemp = {
            "duration": 20,
            "rate": 0.5,
            "temp": "absolute"
        ...
        var output = determine_basal(glucose_status, currenttemp, getNegativeIOB(-0.7), profile, autosens, meal_data, tempBasalFunctions);
    ...
    it('V2 should cancel low-temp when lowish and delta rising faster than BGI', function () {
        var currenttemp = {
            "duration": 20,
            "rate": 0.5,
            "temp": "absolute"
    ...
    it('V2 should set current basal as temp when lowish and delta rising faster than BGI', function () {
        var currenttemp = {
            "duration": 0,
            "rate": 0.5,
            "temp": "absolute"
    ...
    it('V2 should low-temp when low and rising slower than BGI', function () {
        var glucose_status = {
            "delta": 1,
            "glucose": 85,
            "long_avgdelta": 1,
    ...
    it('V2 should high-temp when eventualBG > max_bg', function () {
        var glucose_status = {
            "delta": +3,
            "glucose": 120,
            "long_avgdelta": 0,
    ...
    it('V2 should cancel high-temp when high and avg. delta falling faster than BGI', function () {
        var currenttemp = {
            "duration": 20,
            "rate": 2,
            "temp": "absolute"
    ...
    it('V2 should cancel high-temp when high and delta falling faster than BGI', function () {
        var currenttemp = {
            "duration": 20,
            "rate": 2,
            "temp": "absolute"
    ...
    it('V2 should do nothing when no temp and high and delta falling faster than BGI', function () {
        var currenttemp = {
            "duration": 0,
            "rate": 0,
            "temp": "absolute"
    ...
    it('V2 should high-temp when high and falling slower than BGI', function () {
        var glucose_status = {
            "delta": -1,
            "glucose": 175,
            "long_avgdelta": -1,
    ...
    it('V2 should high-temp when high and falling slowly with low insulin activity', function () {
        var glucose_status = {
            "delta": -1,
            "glucose": 300,
            "long_avgdelta": -1,
        ...
        var output = determine_basal(glucose_status, currenttemp, getIOBForTest(0.5, profile), profile, autosens, meal_data, tempBasalFunctions);
        output.rate.should.be.above(2.5);
        output.duration.should.equal(30);
        output.reason.should.match(/no temp, setting/);
    });

    //it('V2 should set lower high-temp when high and falling almost fast enough with low insulin activity', function () {
    ...
    //it('V2 should reduce high-temp when high and falling almost fast enough with low insulin activity', function () {
    //var glucose_status = {"delta":-6,"glucose":300,"long_avgdelta":-5,"short_avgdelta":-5};
    //var iob_data = {"iob":0.5,"activity":0.005,"bolussnooze":0};
    ...
    it('V2 should profile.current_basal be undefined return error', function () {
    ...
    it('V2 should let low-temp run when bg < 30 (Dexcom is in ???)', function () {
        var currenttemp = {
            "duration": 30,
            "rate": 0,
            "temp": "absolute"
        };
        var output = determine_basal({
            glucose: 10
        }, currenttemp, iob_data, profile, autosens, meal_data, tempBasalFunctions);
        console.log(output);
        (typeof output.rate).should.equal('undefined');
        output.reason.should.match(/CGM is calibrating/);
    });

    it('V2 should cancel high-temp when bg < 30 (Dexcom is in ???)', function () {
        var currenttemp = {
            "duration": 30,
            "rate": 2,
            "temp": "absolute"
    ...
    it('profile should contain min_bg,max_bg', function () {
        var result = determine_basal({
            glucose: 100
        }, undefined, undefined, {
            "current_basal": 0.0
        }, autosens, meal_data, tempBasalFunctions);
    ...
    /*
        it('V2 should return error eventualBG if something went wrong', function () {
    ...
    */
    ...
    //it('V2 should set current basal as temp when low and rising after meal bolus', function () {
    ...
    it('V2 should do nothing when requested temp already running with >15m left', function () {
        var glucose_status = {
            "delta": -2,
            "glucose": 121,
            "long_avgdelta": -1.333,
            "short_avgdelta": -1.333
        };
        ...
        var output = determine_basal(glucose_status, currenttemp, getIOBForTest(3.983, profile), profile, autosens, meal_data, tempBasalFunctions);
        console.log(output);
        (typeof output.rate).should.equal('undefined');
        (typeof output.duration).should.equal('undefined');
    });

    //it('V2 should cancel high temp when low and dropping after meal bolus', function () {
    //var glucose_status = {"delta":-1,"glucose":80,"long_avgdelta":1,"short_avgdelta":1};
    //var iob_data = {"iob":0.5,"activity":-0.01,"bolussnooze":1,"basaliob":-0.5};
    //var currenttemp = {"duration":20,"rate":2,"temp":"absolute"};
    //var output = determine_basal(glucose_status, currenttemp, iob_data, profile, autosens, meal_data, tempBasalFunctions);
    ...
    //it('V2 should cancel low temp when low and rising after meal bolus', function () {
    //var glucose_status = {"delta":1,"glucose":80,"long_avgdelta":1,"short_avgdelta":1};
    //var iob_data = {"iob":0.5,"activity":-0.01,"bolussnooze":1,"basaliob":-0.5};
    //var currenttemp = {"duration":20,"rate":0,"temp":"absolute"};
    //var output = determine_basal(glucose_status, currenttemp, iob_data, profile, autosens, meal_data, tempBasalFunctions);
    ...
    it('V2 should high-temp aggressively when 120 and rising after meal bolus', function () {
    ...
    it('V2 should high-temp aggressively when 150 and rising after meal bolus', function () {
    ...
    it('V2 should reduce high-temp when 160 and dropping slowly after meal bolus', function () {
    ...
    it('V2 should high-temp when 120 and rising after meal bolus', function () {
    ...
    it('V2 should high-temp when 140 and rising after meal bolus', function () {
    ...
    // after 120g 6U meal bolus
    it('V2 should high-temp when 160 and rising after meal bolus', function () {
    ...
    // after 120g 6U meal bolus
    it('V2 should not high-temp when 160 and rising slowly after meal bolus', function () {
    ...
    // after 120g 6U meal bolus
    it('V2 should cancel temp when 160 and falling after meal bolus', function () {
        var glucose_status = {"delta":-1,"glucose":160,"avgdelta":-1};
    ...
    it('V2 should not set temp when boluses + basal IOB cover meal carbs', function () {
        var glucose_status = {"delta":1,"glucose":160,"avgdelta":1};
    ...
    */
    ...
    it('V2 should temp to zero with double sensitivity adjustment', function () {
        //var glucose_status = {"delta":1,"glucose":160,"avgdelta":1};
        var iob_data = {
            "iob": 0.5,
            "activity": 0.001,
    ...
    it('V2 should round appropriately for small basals when setting basal to maxSafeBasal ', function () {
        var glucose_status = {
            "delta": 5,
            "glucose": 185,
            "long_avgdelta": 5,
    ...
    it('V2 should match the basal rate precision available on a 523', function () {
        //var currenttemp = {"duration":30,"rate":0,"temp":"absolute"};
        var currenttemp = {
            "duration": 0,
            "rate": 0,
    ...
    it('V2 should match the basal rate precision available on a 522', function () {
        //var currenttemp = {"duration":30,"rate":0,"temp":"absolute"};
        var currenttemp = {
            "duration": 0,
            "rate": 0,
